...
mod tests {
    ...
    #[test]
    fn process_table_preserves_snapshot_cpu_baseline() {
        let mut collector = Collector::new();
        let baseline = Instant::now();
        collector.cpu_baseline_refreshed_at = Some(baseline);
        let processes = collector.process_table();
        assert_eq!(collector.cpu_baseline_refreshed_at, Some(baseline));
        assert!(
            processes
                .iter()
        ...
        assert!(
            processes
                .iter()
                .all(|process| process.pid > 0 && process.name.len() <= MAX_PROCESS_NAME_BYTES)
        );
