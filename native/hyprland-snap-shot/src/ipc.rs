...
pub fn destination(windows: Vec<Window>, pid: u32, title: &str) -> Result<Option<Window>> {
    ...
    if matches.next().is_some() {
        return Err("More than one Pylon window matches the capture destination.".into());
    }
    Ok(found)
}

pub fn activate(pid: u32, title: &str) -> Result<()> {
    // Subscribe before looking up a newly mapped Pylon window, so no map/title event is missed.
    ...
    loop {
        ...
        if let Some(window) = destination(windows()?, pid, title)? {
            ...
            {
                return Err("Hyprland could not focus Pylon.".into());
            }
            return Ok(());
        }
        events.get_ref().set_read_timeout(Some(
            deadline
                ...
                .ok_or("Pylon did not become visible.")?,
        ))?;
