...
Pylon runs Google's official Antigravity ACP agent. By default it signs in with your personal
...
not sign in this Pylon provider.
...
2. Choose **Install Antigravity**. Pylon downloads the official runtime from Google to that
...
6. Wait for Pylon to confirm sign-in and load the model choices. Select an Antigravity model
...
machine as the Pylon environment, sign-in can finish directly.
...
Return to the same Pylon client and environment where you started sign-in. Another client
...
a thread or bug report. Pylon waits for Google's agent to confirm sign-in. A successful
...
are stored in plain text in Pylon settings on that environment and are passed only to the
...
Pylon does not download it until you choose to install it.

On web or desktop, **Update Antigravity** appears when Pylon has a newer managed release.
...
A nonempty **Binary path** takes priority. With the field empty, Pylon uses its managed
runtime, then an installation on the environment's `PATH`. An invalid explicit path reports
an error instead of selecting another installation. Pylon does not update or remove manual
installations. Clear **Binary path** to use managed installation controls.

## Models and threads

The current official ACP exposes Gemini models only. Pylon uses the model IDs and names
...
Pylon asks you to select an available model instead of silently changing it.

Use Antigravity's native `/plan` command to request a plan. Pylon's separate Plan mode control
is not available for this provider.

Project skills should use `.agents/skills`. Pylon also discovers `.gemini/skills` and the legacy
...
Antigravity reads and edits workspace files through Pylon. Each write shows up as a file
...
When the agent offers **Allow for this thread** on a shell or web tool, Pylon shows Google's
...
Pylon keeps thread history and file diffs. Antigravity does not support conversation rewind,
...
or reply ownership. Pylon cannot show separate child entries or separate child replies
...
installation. Pylon refuses removal while the runtime is in use. An explicit **Binary path**
that points into the managed runtime must be cleared first.

## Account access and errors

Google controls account eligibility, models, and usage limits. Pylon does not report your
...
provider's setup controls on web or desktop. Automatic status checks verify the installation
only.
...
Use any retry time Google supplies. Pylon does not switch to an API key to get past the limit.
