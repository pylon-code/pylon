...
Pylon uses the OpenCode setup on the connected environment. With a remote environment, its
OpenCode login and configuration apply, not the setup on your desktop or phone.

Pylon requires OpenCode 1.14.19 or newer. It checks the server version before it loads models or
...
Without a server URL, Pylon starts a local OpenCode server. The process inherits
`OPENCODE_SERVER_PASSWORD` from the environment. A password in the provider settings overrides
that environment value for both the local process and Pylon.

With a server URL, Pylon connects to that external server and uses only the password in the
...
When you select **Stop**, Pylon stops the main OpenCode session and all nested child sessions.
Pylon waits for this cleanup before it marks the turn as stopped or sends the next prompt. It
...
When Pylon closes an OpenCode session, it also tries to stop the child sessions, but this
...
After changing an OpenCode login or configuration outside Pylon, open **Settings > Providers**,
select the environment, and choose **Refresh provider status**. Changing the provider's
configuration in Pylon also replaces that provider connection.
...
Pylon does not own an external OpenCode server. Native configuration changes on that server can
require its own reload or restart before a refresh returns the new list.

If a refresh fails, Pylon keeps the last known models, slash commands, and skills. Fix the
...
until the model metadata returns. Pylon does not switch the thread to the first model in the
list.
