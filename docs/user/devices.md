...
`agent-device` command line, which Pylon sets up for them.
...
Studio's Device Manager. Pylon detects standard SDK locations; set
...
local network, Tailscale, and Pylon Connect. Live video needs a secure page
...
sessions and stops its Pylon helpers when reachable; simulators keep running.

Pylon provides discovery, streaming, and control. Arrange app builds,
