...
run a headless server from the CLI, or have the desktop app launch Pylon over SSH.
...
Each newly created pairing link row has a **Share** button that expands a panel with a scannable QR code, the full
...
Use this when you want the desktop app to start or reuse Pylon on another machine over SSH.
...
During SSH launch, Pylon first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Pylon uses:
...
the conversation and in **Settings** → **Connections**. Follow the action shown there: Pylon may
...
If Pylon Connect cannot connect, check the date and time on both devices, then try again.

Finish active work before updating because the server restarts briefly. For step-by-step guidance,
see [Keeping Pylon in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running Pylon in the Background](./background-service.md).
...
Hosted pairing does not proxy traffic through Pylon. The browser still connects directly to the backend URL in the pairing link.
...
Open your account menu and choose **Pylon Connect** to see every environment registered to your
account. On mobile, open **Settings** → **Pylon Connect**. Choose **Deregister** to revoke an
environment's Pylon Connect access, remove any managed tunnel, and free its host space.
