...
machine running your Pylon server does that, its themes appear in the theme library alongside
your own, and Pylon follows them: change the desktop theme and Pylon retints with it, without
a restart.
...
follows the machine it is connected to, not the device it runs on. Pylon Mobile keeps its own
appearance settings and does not follow environment themes yet.
...
the standard Pylon look. A theme you saved always wins over a published one with the same id.
...
directory (`~/.pylon-code/userdata/themes/` by default). The filename is the theme id — `nightfall.json`
...
exported from Pylon (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Pylon generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Pylon theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Pylon does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so Pylon never reads
