...
Shift keys together. Pylon attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Pylon is in front captures Pylon itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Pylon refuses shortcuts that
collide with its own keybindings or that the operating system already reserves.
...
terminal contents. If an app is slow to answer, Pylon attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Pylon SnapShots** extension during setup. It is installed
...
GNOME's Extensions app to revoke access. Stable, Nightly, and development builds of Pylon share
this one extension.

**KDE Plasma 6.** Install the bundled capture helper during setup; no sign-out is needed. If the
shortcut does not fire, check Pylon under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Niri.** Choose a shortcut and select **Review changes**. Pylon shows the exact binding it will
...
**Other Wayland desktops.** Pylon uses the desktop's screenshot portal when it can capture the
