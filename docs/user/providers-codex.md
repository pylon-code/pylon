...
This guide is for people who want to use more than one Codex account in Pylon. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Pylon](./install.md).
...
To close an unanswered question without sending a message, choose its dismiss action.
...
reports them. If Codex does not report either value, Pylon leaves it out instead of using the
...
In Pylon Settings, name it something obvious:
...
In Pylon Settings, add another Codex provider:
...
Pylon shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Pylon treats it
