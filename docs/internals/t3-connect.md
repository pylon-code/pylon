...
Pylon Connect uses Clerk for cloud identity. The relay manages environment links,
...
[Pylon Connect setup runbook](../operations/connect-setup.md).
...
redirect ([upstream #5051](https://github.com/pingdotgg/t3code/issues/5051)). The
[shared flow](../../packages/shared/src/connectAuth.ts) preserves PKCE and state
for both loopback and pasted-code callbacks. SSH and headless sessions use the
pasted-code flow because the browser cannot ordinarily reach a listener on the
remote machine.

## Desktop Clerk UI is not pinned

Desktop does not bundle Clerk's UI. The Electron Clerk integration fetches it
from Clerk's CDN at runtime, and Pylon passes no UI version, so desktop requests
the floating `@clerk/ui@1` range on every launch. This is deliberate: it picks up
Clerk's fixes, including the OAuth transfer fixes that motivated dropping the
earlier pin, without a manual bump.

The cost is real. `@clerk/ui` is not a Pylon dependency and has no lockfile entry,
so a new 1.x reaches every desktop user without a commit, release, or CI signal,
and there is nothing to bisect when it breaks. Keep that in mind before assuming
a desktop sign-in regression came from Pylon. The
[setup runbook](../operations/connect-setup.md#desktop-sign-in-breaks-after-a-clerk-ui-release)
has the diagnosis and stopgap pin.
