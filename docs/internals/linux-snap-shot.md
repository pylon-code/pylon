...
hidden desktop entry, not run from the AppImage mount. The install path
...
`<app-id>.SnapShot` on the session bus and exports `com.pylon.code.SnapShot.Capture`; the config
...
Source in `apps/desktop/gnome-extension`, UUID `snap-shot@pylon-code.com`, exporting
`org.gnome.Shell.Extensions.PylonSnapShot`. Every name differs from T3 Code's extension so both apps
can be installed and enabled together. GNOME only discovers a newly
...
The extension trusts callers that own `com.pylon.code.SnapShot` (or the `.nightly` and `.dev`
variants) on the same connection. One extension serves every installed Pylon channel. This is GNOME's trusted-session-client pattern, not authentication
...
inside the extension with coordinates relative to Pylon's content area. Electron 44's restored-session
