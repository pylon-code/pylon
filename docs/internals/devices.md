...
Device panel work over Tailscale and Pylon Connect, including when an SSH host runs the devices.
...
npm-installed at a pinned version into the Pylon home after its matching Device
...
session arbitrary command execution on the host, so Pylon does not. The
...
snapshot model agents need and stays current with its own releases. Pylon prepends
