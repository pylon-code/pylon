...
Every full-screen video preview on iOS uses AVKit. Sent attachments and file links stream
from their signed asset URL; composer drafts play from their retained local file. Inline
...
shared session, which may still serve another player or recorder. Android expanded previews
use a React Native modal with the Expo Video player; inline and workspace videos play in place.
