...
Terminal process polling shares the same monitor. The terminal manager requests one
bounded PID, parent-PID, and process-name table per polling pass for all active
terminals, and requests nothing while no terminal is running. The monitor answers
from a separate `sysinfo::System` so these requests cannot reset the CPU baseline
that resource snapshots measure against. Without the monitor, the manager falls back
to one platform process query per pass, and consecutive native failures double the
polling interval up to 60 seconds even when the fallback succeeds.
