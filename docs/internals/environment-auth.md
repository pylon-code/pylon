...
[Pylon Connect](./t3-connect.md). A relay token is never an environment login.
...
rather than fall back to bearer authentication. Verifier failures cross server and
relay boundaries only as stable safe categories (`time_window`, `key_mismatch`,
`request_mismatch`, `token_mismatch`, `replay`, or `invalid_proof`); older peers omit the
field, and only `time_window` confirms a clock problem. The OAuth token-exchange
