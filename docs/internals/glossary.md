...
The per-driver list of current model slugs that decides which models land in the model picker's legacy section. Bundled at `apps/server/src/provider/model-manifest.json` and refreshed at runtime from Pylon's public releases repository, so classification updates do not need a product release. See the [provider architecture][16] model manifest section.
