...
Pylon has one server-side observability model:
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.pylon-code/userdata/...` by default, or
...
worktree uses `~/.pylon-code/dev/logs/server.trace.ndjson`.
...
export T3CODE_OTLP_SERVICE_NAME=pylon-local
```
...
```bash
TRACE_FILE="${T3CODE_HOME:-$HOME/.pylon-code}/userdata/logs/server.trace.ndjson"
```
...
```bash
TRACE_FILE="$HOME/.pylon-code/dev/logs/server.trace.ndjson"
```
...
- `T3CODE_OTLP_SERVICE_NAME`: service name, default `pylon-server`
