...
Rerunning the latest trusted `pylon` revision also covers manual recovery when the previous run succeeded but the public repository needs reconciliation. Do not add an unrestricted manual-dispatch path that could expose the cross-repository credential to a branch workflow.
