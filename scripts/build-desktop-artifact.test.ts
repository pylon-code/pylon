// @effect-diagnostics nodeBuiltinImport:off - packaged-archive fixtures compute the sidecar digest with the same Node primitive as the builder, and Node's glob matcher verifies electron-builder exclusions.
...
`,
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      const win = yield* createBuildConfig(
        ...
        undefined,
        undefined,
        // localMacSigningIdentity: Pylon-only parameter, absent upstream, so
        // upstream's positional wslRuntimeBundled argument shifts by one.
        undefined,
        true,
      );
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "Pylon (Alpha) 1.2.3 Installer",
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "Pylon", schemes: ["pylon-code"] },
      ]);
      assert.equal(linux.appId, "com.pylon.code");
      assert.equal(linux.artifactName, "Pylon-${version}-${arch}.${ext}");
      assert.equal((linux.linux as Record<string, unknown>).executableName, "pylon");
      assert.equal(
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.pylon.code</string>");
  ...
  it.effect(
    "adds passkey entitlements and the Pylon renderer protocol to signed macOS builds",
    () =>
      Effect.gen(function* () {
