...
branch: main
reviewed-through: "1f8ed54add4133ac39effceded8fc1fff12d8e03"
reviewed-through-date: "2026-08-29"
---
...
2026-11-01 floor is still unreached — and opened DEF-8 and DEF-9. Entries are
...
| DEF-9 | `3d32797f6` / `#8693` — unify activity logs and composer banners        | 2026-08-29  | All three hold. (a) `#184` (web file attachments) has merged, since `#8693` imports `composerAttachmentFiles`. (b) Upstream has been quiet on the new primitives for two weeks: `git log --oneline --since=2026-08-29 t3code-upstream/main -- apps/web/src/components/chat/ComposerBanner.tsx apps/web/src/components/chat/ComposerSurface.tsx apps/web/src/components/chat/ComposerBannerStack.tsx` shows nothing newer than 14 days. (c) The developer has decided which fork task-badge behaviours survive: per-turn task dismiss (`onDismiss`, `ComposerTasksDismissalSnapshot`), `waitingOn`, the timeline `working` row with `workingStepLabel`, and where `ComposerUsageIndicator` sits relative to upstream's `ComposerSurface.ContextStrip`. Not before 2026-09-15. | Manual port: 13 conflicting files, 1180 conflicted lines in `ChatComposer.tsx` alone, six hunks in `ComposerTasksBadge.tsx` where upstream deletes `onDismiss` and Pylon has fork-only dismissal state, a modify/delete on the badge test that `#172` rewrote the day before, and 524 lines of `.chat-composer-*` CSS deleted — safe once consumers migrate, as no fork file references them. Merged upstream 2026-08-29T23:38Z, so follow-ups are likely. It is a UX consolidation, not a fix Pylon is missing.                                                                             |
...
| ------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| WATCH-1 | [`#114`](https://github.com/pylon-code/pylon/issues/114) — Prime provider parity | `#5760` (issue, open) OpenCode todowrite updates do not reach the task sidebar; `#8097` (PR, open) restore composer tasks after relaunch | 2026-08-29 | Both open as of 2026-08-29. Upstream has no provider-neutral task contract; `#8097` carries a migration (`042_ProjectionThreadPlanActivityIndex`) that would collide with Pylon's renumbered migrations. | `#8097` merges, `#5760` closes, or a provider-neutral task contract appears. Check `gh pr view 8097 --repo pingdotgg/t3code --json state,mergedAt`, `gh issue view 5760 --repo pingdotgg/t3code --json state`, and `git log --oneline <cursor>..t3code-upstream/main -- apps/server/src/orchestration/Layers/ProviderRuntimeIngestion.ts apps/web/src/session-logic.ts`. | Compare normalization and recovery behavior against Pylon's managed plan bridge; selectively port only useful patterns. Never take the migration as-numbered. |
