...
- **Pylon Device panel:** when `device_*` tools are available, use `device_list` and `device_open` for live viewing and follow the returned `agent-device` instructions for UI control on either platform. Keep the returned device, host config, and session flags on every command. See [Devices](../../../docs/user/devices.md) for setup. Do not start a separate simulator browser stream or drive the same attached device through another automation tool.
- **iOS:** retain [ios-debugger-agent](../ios-debugger-agent/SKILL.md) for builds and debugging with one explicit UDID. Use its UI automation and screenshots when Pylon device tools are unavailable.
- **Android:** when Pylon device tools are unavailable, prefer semantic automation; otherwise inspect current `uiautomator` hierarchy and use device-scoped `adb` actions. Capture the final state with `adb exec-out screencap -p`. A new streaming infrastructure is unnecessary for screenshots.
