---
name: test-pylon-app
description: Launch, retain, and test the Pylon web app in isolated development environments, including first-try browser authentication with one-time pairing URLs, pairing-token recovery, worktree-safe state directories, cross-turn dev server lifecycle, and direct SQLite inspection or fixture seeding. Use when an agent needs to run Pylon locally, iteratively test UI behavior with a human, recover from an expired or consumed pairing token, isolate dev state, or prepare test data in state.sqlite.
---

# Test Pylon App

Use this skill for the web client. For iOS Simulator, Android Emulator, or physical-device testing against an isolated Pylon backend, use the sibling [`test-pylon-mobile`](../test-pylon-mobile/SKILL.md) skill.
...
2. Choose a base directory that belongs only to the current worktree or test:
   ...
   - Use `mktemp -d /tmp/pylon-test.XXXXXX` for disposable state and retain the printed absolute path.
...
When another person will use the printed pairing URL and browser verification is authorized, open only the shared origin without the pairing path or fragment in the controlled browser and confirm the Pylon app loads. Browsers can block some otherwise reachable ports before making a network request.
...
- Tell the user when a test environment remains available, including its non-secret web URL when useful. Include a fresh pairing URL only when the user still needs to pair (see below); do not repeat its token after pairing succeeds.
...
Treat pairing URLs as secrets: never put one in screenshots, committed files, or durable logs. For an explicit human handoff the deliverable IS the full pairing URL — send it once in chat, token and all, because a bare origin is useless to them. A pairing token is short-lived and single-use; opening the URL in another browser or opening it twice can consume it, so never open a URL you handed to the user.
