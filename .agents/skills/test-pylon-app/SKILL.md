...
- Tell the user when a test environment remains available, including its non-secret web URL when useful. Include a fresh pairing URL only for an explicit human handoff; do not repeat its token after pairing succeeds.
...
Treat pairing URLs as secrets. For an explicit human handoff, send a fresh URL once in chat. Never put it in screenshots, committed files, or durable logs. A pairing token is short-lived and single-use; opening the URL in another browser or opening it twice can consume it.
