...
2. Do independent work first. You may end your turn with the child still pending: while delegation is enabled, Pylon queues a parent follow-through turn when a child completes, stops, errors, or needs approval/input. Busy parents receive consolidated updates at their next eligible idle boundary. For an in-turn wait, use `delegated_thread_status` with `waitSeconds: 45`; completed or blocked children return immediately. Avoid repeated polling, inspections of unfinished files, and unchanged progress updates.
...
Automatic follow-through applies only to Pylon child threads; built-in agents retain provider behavior. Stopped, errored, snoozed, settled, archived, or blocked parents do not automatically resume. Pylon allows at most three automatic follow-through turns since the latest ordinary user message, then records a pause in the work log. A new user message permits further follow-through. Notifications do not authorize retrying a stopped child or approving its actions. Turning delegation off prevents further automatic parent turns without stopping existing children.
