...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the Pylon scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a Pylon terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into Pylon.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before Pylon\'s built-in icon locations.',
      },
  ...
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in Pylon.",
      })
      .check(Schema.isMaxLength(T3_PROJECT_FILE_MAX_SCRIPTS)),
  ),
}).annotate({
  title: "T3 project file",
  description: "Checked-in project configuration for Pylon (t3.json at the repository root).",
});
