...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Built-in Default and Incognito profiles are synthesized rather than stored.
  ...
  // Legacy plan mode. The composer's Build/Plan toggle is off by default; this
  ...
  // Pylon defaults this on where upstream defaults it off: Pylon routes threads
  // across several accounts of one provider, so remaining subscription capacity
  // is routine information rather than a niche readout.
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Pylon spawn the server when needed.",
        providerSettingsForm: {
...
export const PrimeAgentSettings = makeProviderSettingsSchema(
  {
    ...
    agentHomePath: TrimmedString.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "Agent home path",
        description: "Custom Prime Agent home directory. Leave blank to use ~/.prime/agent.",
        providerSettingsForm: {
          placeholder: "~/.prime/agent",
          clearWhenEmpty: "omit",
        },
    ...
    launchArgs: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Launch arguments",
        description:
          "Additional behavior arguments passed on session start. Pylon reserves ACP, session, and model flags.",
  ...
  {
    order: ["binaryPath", "agentHomePath", "launchArgs"],
  },
);
export type PrimeAgentSettings = typeof PrimeAgentSettings.Type;

/** A configured hub reports usage; its management key is redacted for clients. */
...
export const ServerSettings = Schema.Struct({
  ...
  /** Null defers to the newly created checkout's t3.json, then recursive. */
...
const PrimeAgentSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  agentHomePath: Schema.optionalKey(TrimmedString),
  launchArgs: Schema.optionalKey(TrimmedString),
  customModels: Schema.optionalKey(Schema.Array(CustomModelSetting)),
});
