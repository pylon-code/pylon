...
describe("readFilePreviewResponse", () => {
  it("closes a response that arrives after its preview was cancelled", async () => {
    ...
    await expect(readFilePreviewResponse(new Response(body), controller.signal)).rejects.toThrow(
      "Preview cancelled",
    );
