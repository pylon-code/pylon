...
describe("resolveWorkEntryToolPresentation", () => {
  ...
  it("labels device tools with the device icon", () => {
    ...
    expect(resolveWorkEntryToolPresentation({ label: "t3-code · device_screenshot" })).toEqual({
      displayName: "Take a screenshot of the device",
      icon: "device",
    });
  });
  ...
  it.each([
    ...
    ["t3_thread_read", "Reading a Pylon thread", "Read a Pylon thread"],
    ["t3_thread_send", "Sending to a Pylon thread", "Sent to a Pylon thread"],
    [
      "t3_worktree_handoff",
  ...
  it("keeps Pylon branding for non-browser tools and falls back to the original tool label", () => {
    expect(
      resolveWorkEntryToolPresentation({
