...
describe("faviconUrlForPage", () => {
  it("uses the page origin instead of a third-party favicon service", () => {
    ...
    // Pylon refuses private hosts here: a remote client would resolve this
    // against its own network, not the environment that browsed the page.
    expect(faviconUrlForPage("http://localhost:5173/app")).toBeNull();
  });
