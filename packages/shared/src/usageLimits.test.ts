...
describe("local usage command ownership", () => {
  it("handles only the advertised standalone action without attached context", () => {
    expect(shouldHandleUsageLimitsCommand(" /USAGE-LIMITS \n", [USAGE_LIMITS_COMMAND], 0)).toBe(
      true,
    );
    for (const prompt of ["/usage-limits more", "Explain /usage-limits", "/usage-limits\nnext"]) {
      expect(shouldHandleUsageLimitsCommand(prompt, [USAGE_LIMITS_COMMAND], 0)).toBe(false);
    }
    expect(shouldHandleUsageLimitsCommand("/usage-limits", [USAGE_LIMITS_COMMAND], 1)).toBe(false);
    expect(shouldHandleUsageLimitsCommand("/usage-limits", [], 0)).toBe(false);
    expect(
      shouldHandleUsageLimitsCommand(
        "/usage-limits",
        [{ name: "usage-limits", description: "Project command" }],
        0,
      ),
    ).toBe(false);
  });
...
describe("reset credit attempts", () => {
  it("keeps uncertain attempts across retries and separates accounts and environments", () => {
    let sequence = 0;
    const attempts = createResetCreditAttempts(() => `attempt-${++sequence}`);
    const environmentId = EnvironmentId.make("one");
    const input = { instanceId: ProviderInstanceId.make("codex") };
    const first = attempts.begin(environmentId, input);
    expect(attempts.begin(environmentId, input)).toEqual(first);
    expect(attempts.begin(EnvironmentId.make("two"), input)).not.toEqual(first);
    expect(
      attempts.begin(environmentId, { instanceId: ProviderInstanceId.make("codex_work") }),
    ).not.toEqual(first);
    attempts.complete(environmentId, first);
    const next = attempts.begin(environmentId, input);
  ...
  it("keeps the explicit credit identity supplied by a hub", () => {
    const attempts = createResetCreditAttempts(() => {
    ...
    expect(attempts.begin(environmentId, input)).toEqual(input);
    attempts.complete(environmentId, input);
  });

  it("shows a confirmed outcome alongside a refresh warning", () => {
    expect(
      resetCreditOutcomeText({ outcome: "reset", warning: "Could not refresh the balance." }),
    ).toBe("Reset applied. Your windows have cleared. Could not refresh the balance.");
    expect(resetCreditOutcomeText({ outcome: "alreadyRedeemed" })).toBe(
      "That credit was already redeemed.",
    );
  });
});

describe("legacy Pylon window normalization", () => {
  it("matches native Claude and Codex rows with their hub windows", () => {
    expect(
      normalizeUsageWindow(
        { label: "Weekly (all models)", usedPercent: 20, windowDurationMins: 10080 },
        "claudeAgent",
      ),
    ).toMatchObject({ id: "seven_day", kind: "weekly" });
    expect(
      normalizeUsageWindow(
        { label: "Weekly (Fable)", usedPercent: 30, windowDurationMins: 10080 },
        "claudeAgent",
      ),
    ).toMatchObject({ id: "seven_day_fable", kind: "weekly" });
    expect(
      normalizeUsageWindow(
        { label: "Weekly", usedPercent: 40, windowDurationMins: 10080 },
        "codex",
      ),
    ).toMatchObject({ id: "secondary", kind: "weekly" });
    expect(
      normalizeUsageWindow(
        { label: "Monthly", usedPercent: 50, windowDurationMins: 43200 },
        "codex",
      ),
    ).toMatchObject({ id: "primary", kind: "monthly" });
  });

  it("preserves unknown providers and explicit ids", () => {
    expect(
      normalizeUsageWindow(
        { id: "custom", kind: "other", label: "Credits", usedPercent: 40 },
        "primeAgent",
      ),
    ).toMatchObject({ id: "custom", kind: "other" });
    expect(normalizeUsageWindow({ label: "Window", usedPercent: 40 }, "primeAgent")).toMatchObject({
      id: "Window",
      kind: "other",
    });
  });
...
describe("collectLimitSources", () => {
  ...
  it("keys sources per environment and names the environment only when several have some", () => {
    ...
    expect(collectLimitSources(one as never).map((entry) => [entry.key, entry.label])).toEqual([
      [JSON.stringify(["source", "env-a", "cliproxy-hub"]), "hub"],
    ]);
...
describe("pools", () => {
  ...
  it("does not confuse a newer window push with a newer reset-credit balance", () => {
    const staleCredit = provider({
      auth: { status: "authenticated", email: "same@example.com" },
      usageLimits: {
        checkedAt: "2026-09-03T11:45:00.000Z",
        windows: [{ ...window, usedPercent: 50 }],
        resetCredits: { availableCount: 2, checkedAt },
      },
    });
    const freshCredit = {
      ...staleCredit,
      usageLimits: {
        ...
        resetCredits: { availableCount: 0, checkedAt: "2026-09-03T11:30:00.000Z" },
      },
    };
    const [account] = collectLimitAccounts(
      new Map([
        [EnvironmentId.make("env-a"), { ...laptop, serverConfig: { providers: [staleCredit] } }],
        [EnvironmentId.make("env-b"), { ...laptop, serverConfig: { providers: [freshCredit] } }],
      ]),
    );
    expect(account?.limits.windows[0]?.usedPercent).toBe(50);
    expect(account?.limits.resetCredits?.availableCount).toBe(0);
    expect(account?.redeem?.environmentId).toBe("env-b");
  });

  it("keeps hub source and account ids with colon-delimited parts distinct", () => {
    const account = { id: "c", driver: claude, usageLimits: { checkedAt, windows: [window] } };
    const sources = [
      { ...source, id: UsageLimitSourceId.make("a:b"), accounts: [account] },
      { ...source, id: UsageLimitSourceId.make("a"), accounts: [{ ...account, id: "b:c" }] },
    ];
    const accounts = collectLimitAccounts(
      new Map([
        [EnvironmentId.make("env-a"), { ...laptop, serverConfig: { usageLimitSources: sources } }],
      ]),
    );
  ...
  it("merges one account reported natively on two environments and by a hub into one entry", () => {
    ...
    expect(accounts[0]).toMatchObject({
      key: JSON.stringify(["native", "env-a", "claude"]),
      sourceLabel: null,
  ...
  it("keeps unidentified hub accounts separate across environments", () => {
    const seat = {
    ...
    expect(accounts.map((account) => account.key)).toEqual([
      JSON.stringify(["hub", "env-a", "hub", "claude-team-seat.json"]),
      JSON.stringify(["hub", "env-b", "hub", "claude-team-seat.json"]),
    ]);
  ...
  it("pools windows by id across accounts and orders resets by when they land", () => {
    ...
    const untimed = collectLimitPools(
      collectLimitAccounts(input).map((account) =>
        account.key === JSON.stringify(["hub", "env-a", "hub", "b"])
          ? {
              ...account,
              limits: {
                ...account.limits,
    ...
    expect(
      ...
      [JSON.stringify(["hub", "env-a", "hub", "a"]), 40],
      [JSON.stringify(["hub", "env-a", "hub", "b"]), 20],
    ]);
    ...
    expect(session?.members.map((member) => member.account.key)).toEqual([
      JSON.stringify(["hub", "env-a", "hub", "a"]),
      JSON.stringify(["hub", "env-a", "hub", "b"]),
    ]);
...
describe("pooled account columns", () => {
  ...
  it("orders by the session reset when an older server omits window ids and kinds", () => {
    const { id: _id, kind: _kind, ...legacySession } = window;
    const { id: _weeklyId, kind: _weeklyKind, ...legacyWeekly } = weekly;
    const [pool] = collectLimitPools(
      [
        account("a", [
          { ...legacyWeekly, resetsAt: "2026-09-05T12:00:00.000Z" },
          { ...legacySession, resetsAt: "2026-09-03T15:00:00.000Z" },
        ]),
        account("b", [
          { ...legacyWeekly, resetsAt: "2026-09-06T12:00:00.000Z" },
          { ...legacySession, resetsAt: "2026-09-03T13:00:00.000Z" },
        ]),
      ],
      now,
    );
...
describe("/usage-limits", () => {
  ...
  it("uses hub credit balances and redemption targets in the composer, including native duplicates", () => {
    ...
    expect(
      report?.accounts.find((account) => account.id === JSON.stringify(["hub", "hub", "oss"]))
        ?.resetCreditInput,
    ).toEqual({
      sourceId: "hub",
      accountId: "oss",
  ...
  it("supports a source-only provider and keeps duplicates when the native probe failed", () => {
    ...
    ).toEqual([JSON.stringify(["hub", "hub", "duplicate"]), JSON.stringify(["hub", "hub", "oss"])]);
    ...
    expect(
      ...
      "codex",
      JSON.stringify(["hub", "hub", "duplicate"]),
      JSON.stringify(["hub", "hub", "oss"]),
    ]);
  ...
  it("advertises global and workspace commands only for providers present in Limits", () => {
    ...
    expect(withUsageLimitsCommands([ownCommand], sources)[0]?.slashCommands).toEqual(
      ownCommand.slashCommands,
    );
    const workspaceCommand = { name: "USAGE-LIMITS", description: "Project command" };
      ...
      workspaceSnapshots: [
        {
          cwd: "/tmp/project",
          checkedAt: limits.checkedAt,
          slashCommands: [workspaceCommand],
          skills: [],
        },
      ],
    };
    expect(
      withUsageLimitsCommands([workspaceOwned], sources)[0]?.workspaceSnapshots?.[0]?.slashCommands,
    ).toEqual([workspaceCommand]);
    expect(supported?.slashCommands[0]?.localAction).toBe("usage-limits");
    const ownSkill = { name: "usage-limits", path: "/skills/usage-limits/SKILL.md", enabled: true };
    expect(
      withUsageLimitsCommands([{ ...withWorkspace, skills: [ownSkill] }], sources)[0]
        ?.slashCommands,
    ).toEqual([]);
    expect(
      withUsageLimitsCommands(
        ...
        sources,
      )[0]?.workspaceSnapshots?.[0]?.slashCommands,
    ).toEqual([]);
