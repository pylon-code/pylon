...
export function faviconUrlForPage(rawUrl: string | null | undefined, _size = 32): string | null {
  ...
  try {
    ...
    // Same disclosure guard as `faviconUrlForOrigin`: a work log must not make
    // the client fetch from a private or reserved host.
    if (!isPublicFaviconHost(pageUrl.hostname)) return null;
