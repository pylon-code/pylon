...
describe("attachment preview classification", () => {
  it.each([
    ["source.ts", "video/mp2t", "text"],
    ["recording", "video/mp2t", "unsupported"],
    ["recording.mp4", "video/x-unknown", "unsupported"],
    ["recording", "VIDEO/WebM; codecs=vp9", "video"],
    ["recording.mp4", "application/pdf", "pdf"],
  ] as const)("keeps Pylon media classification for %s (%s)", (name, mimeType, expected) => {
