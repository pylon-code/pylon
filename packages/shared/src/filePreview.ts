...
export function filePreviewKind(file: {
  ...
  if (mime.startsWith("video/")) {
    if (videoMimeType({ name: file.name, mimeType: mime }) !== null) return "video";
    // Host MIME tables label TypeScript as MPEG transport streams. Offer the
    // bounded text reader; actual binary streams still fail text validation.
    return mime === "video/mp2t" && extension === ".ts" ? "text" : "unsupported";
  }
