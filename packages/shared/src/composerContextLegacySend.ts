 ...
 *
 * Known records use the format `upgradeLegacyContextMessage` parses. Future payloads remain
 * readable as escaped text because an older server cannot persist their structured records.
 */
export function serializeLegacyContextMessage(input: {
  ...
  // Known references become legacy tokens or labels; captured payloads follow in the blocks
  // used by the old composer. Future references retain their generic provider envelope.
  let text = input.text;
  const occurrences = collectComposerContextReferences(text);
  const genericReferences = occurrences.filter((occurrence) => {
    const record = recordsById.get(occurrence.contextId);
    return !record || "payload" in record;
  });
  // Select original references before inserting captured text, which may itself contain literal
  // context links. Those captured links are data and must never resolve another record.
  const genericContext = projectComposerContextForProvider({
    text: genericReferences
      .map((occurrence) => text.slice(occurrence.start, occurrence.end))
      .join("\n"),
...
function inlineLabel(record: ComposerContextRecord): string {
  if (record.kind === "mention" && "path" in record) {
    const escapedPath = record.path.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
    return '@"' + escapedPath + '"';
  }
  if (record.kind === "skill" && "name" in record) {
    return "$" + record.name;
  }
