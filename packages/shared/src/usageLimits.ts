...
export function resetCreditOutcomeText(result: ProviderConsumeResetCreditResult): string {
  const outcome = {
  ...
  return result.warning ? `${outcome} ${result.warning}` : outcome;
}

export type UsageWindowKind = NonNullable<ServerProviderUsageWindow["kind"]>;
type NamedUsageWindow = ServerProviderUsageWindow & {
  readonly id: string;
  readonly kind: UsageWindowKind;
};

/** Older Pylon servers identify windows by duration and label. Keep those readings visible. */
export function normalizeUsageWindow(
  ...
  const kind =
    window.kind ??
    (minutes !== undefined && minutes >= 30 * 24 * 60
      ? "monthly"
      : minutes !== undefined && minutes >= 7 * 24 * 60
        ? "weekly"
        : minutes !== undefined && minutes < 24 * 60
          ? "session"
          : "other");
  const scopedName = /^Weekly\s*(?:\((?!all models\))(.+)\)|·\s*(.+))$/i.exec(window.label);
  const scopedId = scopedName
    ? `seven_day_${(scopedName[1] ?? scopedName[2]!).toLowerCase().replace(/[^a-z0-9]+/g, "_")}`
    : undefined;
  const id =
    window.id ??
    scopedId ??
  ...
  return perEnvironment.flatMap(({ environmentId, environmentLabel, sources }) =>
    sources.map((source) => {
      ...
      return {
        ...
        environmentId,
        key: JSON.stringify(["source", environmentId, source.id]),
...
function creditReadAt(limits: ServerProviderUsageLimits): number {
  return Date.parse(limits.resetCredits?.checkedAt ?? limits.checkedAt);
}
...
export function collectLimitAccounts(
  ...
  const merge = (key: string, next: LimitAccount) => {
    ...
    if (
      ...
      (!previousHub || creditReadAt(next.limits) > creditReadAt(previousHub.limits))
    ) {
    ...
    if (
      ...
      (!previousCredit || creditReadAt(next.limits) > creditReadAt(previousCredit.limits))
    ) {
...
export interface LimitPoolMember {
  ...
  readonly window: NamedUsageWindow;
}
...
export interface LimitPoolWindow {
  ...
  readonly columns: ReadonlyArray<{
    readonly account: LimitAccount;
    readonly window: NamedUsageWindow | null;
  }>;
...
export function collectLimitPools(
  ...
  return [...byDriver].map(([driver, members]) => {
    // Older servers omit window ids and kinds, so order by the same names the pools use.
    const namedWindows = (account: LimitAccount) =>
      account.limits.windows.map((window) => normalizeUsageWindow(window, account.driver));
    const orderWindow = members
      .flatMap(namedWindows)
      .sort((left, right) => WINDOW_KIND_ORDER[left.kind] - WINDOW_KIND_ORDER[right.kind])[0];
    const orderReset = (account: LimitAccount) => {
      const window = namedWindows(account).find(
...
function poolWindows(accounts: readonly LimitAccount[], now: number): readonly LimitPoolWindow[] {
  ...
  for (const account of accounts) {
    for (const reading of account.limits.windows) {
      const window = normalizeUsageWindow(reading, account.driver);
...
/** Limit commands are served by Pylon from the same snapshots as Usage → Limits. */
export const USAGE_LIMITS_COMMAND = {
  name: "usage-limits",
  localAction: "usage-limits",
...
/** Only the advertised local action owns a plain command, never provider commands or attached context. */
export function shouldHandleUsageLimitsCommand(
  prompt: string,
  ...
  return (
    ...
    commands.find((command) => command.name.toLowerCase() === "usage-limits")?.localAction ===
      "usage-limits"
  );
}
...
export function withUsageLimitsCommands(
  ...
  return providers.map((provider) => {
    ...
    const commands = (
      items: readonly ServerProviderSlashCommand[],
      skills: ServerProvider["skills"],
    ) =>
      items.some((command) => command.name.toLowerCase() === USAGE_LIMITS_COMMAND.name) ||
      skills.some(
        (skill) =>
          skill.enabled &&
          skill.userInvocable !== false &&
          skill.name.trim().toLowerCase() === USAGE_LIMITS_COMMAND.name,
      )
        ? items
        : [...items, USAGE_LIMITS_COMMAND];
    return {
      ...provider,
      slashCommands: commands(provider.slashCommands, provider.skills),
      ...(provider.workspaceSnapshots
        ? {
            workspaceSnapshots: provider.workspaceSnapshots.map((snapshot) => ({
              ...snapshot,
              slashCommands: commands(snapshot.slashCommands, snapshot.skills),
            })),
          }
        : {}),
    };
...
export function collectProviderUsageLimits(
  ...
  for (const provider of native) {
    ...
    const hubCredits = sources
      ...
      .sort((a, b) => creditReadAt(b.account.usageLimits) - creditReadAt(a.account.usageLimits))[0];
