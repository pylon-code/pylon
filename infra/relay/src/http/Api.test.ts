...
describe("relay routing fallback", () => {
  it.effect("publishes activity for escaped delegated thread IDs longer than 100 characters", () =>
    Effect.gen(function* () {
      ...
      const overLimit = HttpServerRequest.fromWeb(
        new Request(
          `https://relay.test/v1/environments/${environmentId}/threads/${"t".repeat(513)}/agent-activity`,
          {
            method: "POST",
            headers: {
              authorization: "Bearer environment-credential",
      ...
      const overLimitResponse = yield* httpEffect.pipe(
        Effect.provideService(HttpServerRequest.HttpServerRequest, overLimit),
      );
      expect(overLimitResponse.status).toBe(404);
