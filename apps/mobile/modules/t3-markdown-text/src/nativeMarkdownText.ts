  ...
  switch (node.type) {
    ...
    case "link": {
      ...
      if (reference) {
        ...
        appendChildren(referenceRuns, node, { ...context, href: node.href });
        // A styled label is still one atomic chip. Flatten its visible text,
        // while retaining separate runs for adjacent links with the same href.
        const chipRuns: NativeMarkdownTextRun[] = [];
        appendRun(chipRuns, referenceRuns.map((run) => run.text).join(""), {
          ...context,
          href: node.href,
          fileIcon:
