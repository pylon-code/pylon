...
private final class ComposerTextView: UITextView {
  ...
  @objc private func pasteInline(_ sender: UIKeyCommand) {
    ...
    // A structured Pylon clipboard also has a plain-text representation.
    // Insert that text directly so Paste as Text never imports its context
    // fragment or selects the image/HTML path in paste(_:).
  ...
  override func paste(_ sender: Any?) {
    ...
    if let text = pasteboard.string, shouldInterceptTextPaste(text) {
