...
class T3ComposerEditorView(context: Context, appContext: AppContext) : ExpoView(
      ...
      GestureDetector(
        ...
        object : GestureDetector.SimpleOnGestureListener() {
          ...
          override fun onSingleTapUp(event: MotionEvent): Boolean {
            val token = contextTokenAt(event.x, event.y) ?: return false
  ...
  private fun contextTokenAt(x: Float, y: Float): ComposerToken? {
    ...
    val layoutX = x + editor.scrollX - editor.totalPaddingLeft
    val layoutY = y + editor.scrollY - editor.totalPaddingTop
    return tokens.firstOrNull { token ->
      (token.type == "context" || token.type == "mention" || token.type == "skill") &&
        token.start >= 0 && token.end > token.start && token.end <= content.length &&
        content.substring(token.start, token.end) == token.source &&
        content.getSpans(token.start, token.end, ComposerChipSpan::class.java).any { span ->
          content.getSpanStart(span) == token.start && content.getSpanEnd(span) == token.end &&
            span.containsPoint(layout, token.start, token.end, editor.paint, layoutX, layoutY)
        }
    }
  }
...
private class ComposerChipSpan(
  ...
  fun containsPoint(
    ...
    // Caret hit testing snaps the right half of a ReplacementSpan to its end. Use the
    // selected glyph area instead, retaining bidi boundaries and rejecting adjacent chips.
    val path = Path()
    layout.getSelectionPath(start, end, path)
    val top = layout.getLineTop(line)
    val bottom = layout.getLineBottom(line)
    // An end offset at a soft wrap selects padding through the rest of the line and
    // into the next one. Only the occupied text on the chip's own line is clickable.
    val clip = Region(kotlin.math.floor(left).toInt(), top, kotlin.math.ceil(right).toInt(), bottom)
    val selected = Region()
    selected.setPath(path, clip)
    return selected.contains(kotlin.math.floor(x).toInt(), top + (bottom - top) / 2)
  }
...
internal class SelectionAwareEditText(context: Context) : EditText(context) {
  ...
  override fun onTextContextMenuItem(id: Int): Boolean {
    ...
    val handled = when {
      ...
      // Explicit Paste as Text keeps Android's native insertion and undo, even above the
      // message limit. The send guard reports an over-limit draft without changing its bytes.
      id == android.R.id.pasteAsPlainText -> false
