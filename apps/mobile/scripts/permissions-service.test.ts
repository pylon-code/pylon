...
import { supportsNativeThreadSanitizer } from "./native-thread-sanitizer";
...
it.skipIf(NodeOS.platform() !== "darwin")(
  ...
  () => {
    ...
    try {
      ...
      const sanitizerAvailable = supportsNativeThreadSanitizer("clang");
      NodeChildProcess.execFileSync(
        "clang",
        [
          "-fobjc-arc",
          "-fblocks",
          ...(sanitizerAvailable ? ["-fsanitize=thread"] : []),
          "-framework",
          "Foundation",
          source,
          "-o",
      ...
      expect(
        NodeChildProcess.execFileSync(executable, {
          encoding: "utf8",
          timeout: 15_000,
