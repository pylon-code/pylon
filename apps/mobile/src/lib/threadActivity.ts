...
function workLogEntryIsToolLike(entry: WorkLogEntry | DerivedWorkLogEntry): boolean {
  if ("sourceActivityKind" in entry && entry.sourceActivityKind === "turn.response.missing")
    return false;
...
function toThreadFeedActivityEntry(
  ...
  return {
    ...
    activity: {
      ...
      ...(entry.sourceActivityKind === "turn.response.missing"
...
/** Orders Pylon session-interaction activities before reducing their lifecycle. */
