...
async function importAttachment(
  ...
  const result = await executeAtomQuery(
    ...
    { refresh: true, reportFailure: false, signal },
  );
  ...
  if (Option.isNone(currentConnection) || currentConnection.value !== connection.value) {
    throw new Error("The source environment changed. Copy the attachment again.");
  }
