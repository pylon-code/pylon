...
/** Inline context IDs and their captured attachment IDs diverge after copying or uploading. */
export function referencedComposerAttachmentIds(
  text: string,
  context?: OrchestrationMessageContext,
) {
  return new Set(
    referencedComposerContext(text, context)?.records.flatMap((record) =>
...
export function reidentifyComposerContext(
  ...
  const ids = new Map<ComposerContextId, ComposerContextId>();
  const importedId = (id: ComposerContextId) => {
    const existing = ids.get(id);
    if (existing) return existing;
    const fresh = ComposerContextId.make(createId());
    ids.set(id, fresh);
    return fresh;
  };
  for (const record of records) importedId(record.contextId);
  return {
    text: replaceComposerContextReferences(text, (ref) =>
      formatComposerContextReference({
        ...ref,
        // Missing payloads need fresh identities too: they must stay unavailable,
        // rather than bind to an unrelated same-ID record already in the draft.
        contextId: importedId(ref.contextId),
      }),
    ),
    context: {
      version: 1 as const,
      records: records.map((record) => ({
        ...record,
        contextId: importedId(record.contextId),
        ...(record.kind === "preview-annotation" &&
        "screenshotContextId" in record &&
        record.screenshotContextId
          ? {
              screenshotContextId: importedId(record.screenshotContextId),
            }
          : {}),
      })),
    },
