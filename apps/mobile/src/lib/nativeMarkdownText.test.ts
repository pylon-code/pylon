...
describe("nativeMarkdownDocumentRuns", () => {
  ...
  it("keeps one chip and copied reference for a label containing inline emphasis", () => {
    const href = "t3-context://v1/file/example";
    const runs = nativeMarkdownDocumentRuns({
      type: "document",
      children: [
        {
          ...
          children: [
            {
              ...
              children: [
                ...
                { type: "bold", children: [{ type: "text", content: "final" }] },
    ...
    expect(runs).toEqual([{ text: "report final.pdf", role: "body", href, fileIcon: "text" }]);
    expect(
      nativeMarkdownContextCopyRanges(runs.map((run) => ({ run, text: "", inlineImageLength: 1 }))),
    ).toEqual([{ start: 0, end: 1, text: "[report final.pdf](t3-context://v1/file/example)" }]);
  });

  it("preserves directories in an inline code context label", () => {
    const href = "t3-context://v1/file/example";
    const runs = nativeMarkdownDocumentRuns({
      type: "document",
      children: [
        {
          type: "paragraph",
          children: [
            { type: "link", href, children: [{ type: "code_inline", content: "src/main.ts" }] },
          ],
        },
      ],
    });
    expect(runs).toEqual([{ text: "src/main.ts", role: "body", href, fileIcon: "text" }]);
    expect(
      nativeMarkdownContextCopyRanges(runs.map((run) => ({ run, text: "", inlineImageLength: 1 }))),
    ).toEqual([{ start: 0, end: 1, text: "[src/main.ts](t3-context://v1/file/example)" }]);
  });
});
