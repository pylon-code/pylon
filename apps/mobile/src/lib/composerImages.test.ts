...
describe("composerStripAttachments", () => {
  ...
  it("keeps media, because a thumbnail is the only way to see it", async () => {
    ...
    const kept = composerStripAttachments([image, video] as never, new Set());
    expect(kept.map((a) => a.id)).toEqual(["img-1", "vid-1"]);
  });

  it("shows a document above the composer only when it has no inline chip", async () => {
    const { composerStripAttachments } = await import("./composerImages");
    expect(composerStripAttachments([doc] as never, new Set([doc.id]))).toEqual([]);
    expect(composerStripAttachments([doc] as never, new Set())).toEqual([doc]);
  });
