...
describe("buildThreadFeed", () => {
  ...
  it("attaches reported cost only to turn messages and not the work log", () => {
    ...
    expect(buildThreadFeed(thread)[0]).toBe(first[0]);
    const updated = buildThreadFeed({
      ...thread,
      activities: thread.activities.map((activity) =>
        activity.kind === "turn.cost" ? { ...activity, payload: { totalCostUsd: 0.5 } } : activity,
      ),
    });
    expect(updated[0]).toMatchObject({
      message: thread.messages[0],
      reportedCostLabel: "Reported cost $0.5000",
    });
    expect(updated[0]).not.toBe(first[0]);
    expect(first[0]).toMatchObject({ reportedCostLabel: "Reported cost $0.1235" });
  });
      ...
      expect(terminalRows).toMatchObject([
        {
          ...
          hasFailure,
          live: true,
          // Completed calls retain their outcome while Thinking owns ongoing turn work.
          shimmer: false,
        },
        {
          type: "activity-group",
  ...
  it("names the still-running tool at the active tail, not the finished one", () => {
    ...
    const statuslessRows = deriveThreadFeedPresentation(
      [
        {
          ...feed[0]!,
          type: "activity-group",
          turnId,
          activities: [
            make("act-1", undefined, "pnpm build"),
            make("act-2", "completed", "git status"),
          ],
        },
      ],
    ...
    expect(statuslessRows[0]).toMatchObject({
      type: "work-toggle",
      summary: "Ran git",
      live: true,
      shimmer: false,
...
describe("quiet timeline: nested agents", () => {
  ...
  it("summarizes a spawn card from the newest member report and the batch outcome", () => {
    ...
    const duplicateNames = agentSpawnSummary(
      direct([
        member("Reviewer", "inProgress", "first", 1),
        member("Reviewer", "inProgress", "second", 2),
      ]),
      "inProgress",
    );
    expect(duplicateNames.members.map((member) => member.taskId)).toEqual(["a0", "a1"]);
    expect(
      agentSpawnSummary(
        direct([
          member("Renamed reviewer", "completed", "done", 3),
          member("Reviewer", "inProgress", "second", 2),
        ]),
        "inProgress",
