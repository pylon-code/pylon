...
describe("mobile composer context", () => {
  ...
  it("finds captured attachment IDs independently of copied reference IDs and restores the strip when removed", () => {
    expect(
      referencedComposerAttachmentIds(formatComposerContextReference(image), {
        version: 1,
        records: [image],
      }),
    ).toEqual(new Set(["local-image"]));
    expect(
      referencedComposerAttachmentIds("reference removed", { version: 1, records: [image] }),
    ).toEqual(new Set());
    expect(referencedComposerAttachmentIds(formatComposerContextReference(image))).toEqual(
      new Set(),
    );
  });
  ...
  it("reidentifies an unavailable screenshot even when its record is absent", () => {
    let next = 0;
    const imported = reidentifyComposerContext(
      formatComposerContextReference(annotation),
      [annotation],
      () => `copy-${++next}`,
    );
    expect(imported.context.records).toEqual([
      { ...annotation, contextId: "copy-1", screenshotContextId: "copy-2" },
    ]);
    expect(
      imported.context.records.some(
        (record) => record.contextId === annotation.screenshotContextId,
      ),
    ).toBe(false);
  });
