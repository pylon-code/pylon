...
import type { NativeContextClipboard } from "../lib/composerContextClipboard";
import { importComposerContextIntoDraft } from "../state/import-composer-context";
...
export function ComposerEditor({
  ...
  const pasteContext = async (clipboard: NativeContextClipboard) => {
    ...
    const finishImport = beginComposerContextImport(draftKey);
    if (!finishImport) return;
    const controller = new AbortController();
    importRef.current = controller;
    setImporting(true);
    try {
      const result = await importComposerContextIntoDraft(draftKey, clipboard, controller.signal);
      if (result.status === "stale") {
        if (!controller.signal.aborted)
          Alert.alert("Draft changed", "Paste again to add this context to the updated draft.");
        return;
      }
