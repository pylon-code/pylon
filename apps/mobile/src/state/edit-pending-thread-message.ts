...
export async function editPendingThreadMessage(message: QueuedThreadMessage): Promise<boolean> {
  ...
  try {
    ...
    // Same provider binding a held-send restore records: the queued choice is
    // the user's, so it must not be re-seeded from thread defaults.
    updateComposerDraftSettings(draftKey, {
      ...(message.modelSelection
        ? { modelSelection: message.modelSelection, providerSelectionExplicit: true }
        : {}),
