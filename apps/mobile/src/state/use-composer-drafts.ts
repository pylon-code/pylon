...
export function beginComposerContextImport(draftKey: string): (() => void) | null {
  const next = { ...appAtomRegistry.get(composerContextImportsAtom) };
  if (next[draftKey]) return null;
  next[draftKey] = true;
  ...
  return () => {
    if (finished) return;
    finished = true;
    const current = { ...appAtomRegistry.get(composerContextImportsAtom) };
    delete current[draftKey];
    appAtomRegistry.set(composerContextImportsAtom, current);
  };
}
...
/** Clipboard fragments use either a draft ID or its upload ID, while local bytes may still exist. */
export function findLocalComposerClipboardAttachment(
  environmentId: EnvironmentId,
  id: string,
): DraftComposerAttachment | undefined {
  const matchesId = (attachment: DraftComposerAttachment) =>
    attachment.id === id ||
    (attachment.uploadEnvironmentId === environmentId && attachment.uploadedAttachmentId === id);
...
export function restorePendingSendComposerDraftState(
  ...
  const restored: ComposerDraft = {
    ...
    context: mergeReferencedComposerContext(
      mergeComposerDraftText(existing.text, snapshot.text),
      existing.context,
      snapshot.context,
    ),
  ...
  if (
    ...
    existing.context === restored.context &&
...
export function sameComposerDraftState(a: ComposerDraft, b: ComposerDraft): boolean {
  return (
    ...
    a.providerSelectionExplicit === b.providerSelectionExplicit &&
