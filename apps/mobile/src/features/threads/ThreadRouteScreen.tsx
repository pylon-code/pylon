...
export function ThreadRouteScreen(props: ThreadRouteScreenProps) {
  ...
  const recovery = threadRouteRecovery({
    hasEnvironmentRuntime: routeEnvironmentRuntime !== null,
    connectionState: routeConnectionState,
    shellStatus: routeEnvironmentShellState.status,
    detailStatus: selectedThreadDetailState.status,
    detailHasError: Option.isSome(selectedThreadDetailState.error),
  ...
  return (
    <ThreadUnavailableScreen
      actionLabel={
        recovery === "view-threads"
          ? "View threads"
          : recovery === "manage-environments"
            ? "Manage environments"
            : "Reconnect environment"
      }
      onAction={() => {
        if (recovery === "view-threads") {
          navigation.dispatch(StackActions.replace("Home"));
          return;
        }
        if (recovery === "reconnect-environment") {
...
function ThreadRouteContent(
    ...
    navigation.dispatch(
      StackActions.replace("NewTaskSheet", {
        ...
        params: {
          environmentId: String(creation.environmentId),
          projectId: String(creation.creation.projectId),
          pendingTaskId: String(creation.messageId),
          ...(selectedThreadProject ? { title: selectedThreadProject.title } : {}),
        },
      }),
    );
  ...
  const creationState = ((): ThreadDetailScreenProps["creationState"] => {
    ...
    const hold =
      selectedThreadCreation.outcome === null
        ? selectedThreadCreation.message.deliveryHold
        : undefined;
    if (hold !== undefined) {
      return {
        kind: "failed",
        reason: hold.reason,
        note: "The task is held on this device until you edit it.",
        onEditTask: handleEditHeldCreation,
      };
    }
