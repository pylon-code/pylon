    ...
    >
      ...
      {notice ? (
        ...
        <View className="gap-3">
          {limits.windows
            .map((reading) => normalizeUsageWindow(reading, props.driver))
            .map((window) => (
...
const resetAttempts = createResetCreditAttempts(uuidv4);
...
export function ResetCredits(props: {
  ...
  const redeem = async () => {
    if (redeeming.current || activeTarget.current !== targetKey) return;
    redeeming.current = true;
    setBusy(true);
    setStatus(null);
    try {
      const attempt = resetAttempts.begin(environmentId, input);
      const result = await consume({ environmentId, input: attempt });
      if (result._tag === "Success") resetAttempts.complete(environmentId, attempt);
      if (activeTarget.current !== targetKey) return;
      setStatus(
        result._tag === "Success"
          ? resetCreditOutcomeText(result.value)
          : "error" in result.cause && result.cause.error instanceof Error
            ? result.cause.error.message
            : "Could not use the reset credit. Retry to check the same attempt.",
      );
    } catch {
      if (activeTarget.current === targetKey) {
        setStatus("Could not use the reset credit. Retry to check the same attempt.");
      }
    } finally {
      if (activeTarget.current === targetKey) {
        redeeming.current = false;
