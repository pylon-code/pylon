...
it.layer(integrationLayer)("AgentSessionImporter integration", (it) => {
  it.effect("imports once after the real engine persists an old rejected receipt", () =>
    Effect.gen(function* () {
      ...
      // Pylon's rollback completion is a revision compare-and-set, so the
      // revert needs a completed turn after the imported history to undo.
      yield* engine.dispatch({
        type: "thread.turn.diff.complete",
        commandId: CommandId.make("complete-turn-after-import"),
        threadId,
        turnId: TurnId.make("turn-after-import"),
        completedAt: "2026-08-24T10:04:00.000Z",
        checkpointRef: CheckpointRef.make("refs/t3/checkpoints/import/turn/1"),
        status: "ready",
        files: [],
      ...
      yield* engine.dispatch({
        ...
        threadId,
        operationId: "revert-imported-thread-to-baseline",
        sourceRevision: 1,
        targetRevision: 0,
        turnCount: 0,
