    ...
    Effect.gen(function* () {
      ...
      yield* broker.focusHost({
        ...
        liveTabs: [
          ...
          { threadId: scope.threadId, tabId: PreviewTabId.make("other-signed-in"), visible: false },
          {
      ...
      const wrongHostError = yield* broker
        .invoke<string>({
          scope,
          tabId: PreviewTabId.make("other-signed-in"),
          operation: "evaluate",
          input: {},
        })
        .pipe(Effect.flip);
...
it.effect("retains capability-rich host preference when no host owns the target tab", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const broker = yield* makeBroker;
...
it.effect("fails closed when a named tab's owner cannot perform the requested operation", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const broker = yield* makeBroker;
      ...
      for (const [clientId, supportedOperations] of [
        ["owner", ["status"]],
        ["other", ["status", "snapshot"]],
      ] as const) {
        const requests = requestsFrom(
          yield* broker.connect(makeHost({ clientId, supportedOperations })),
          (connectionId) => {
            if (clientId === "owner") ownerConnectionId = connectionId;
          },
        );
        yield* Stream.runForEach(requests, (request) =>
          broker.respond({
      ...
      yield* Effect.yieldNow;
      const tabId = PreviewTabId.make("named-tab");
      yield* broker.focusHost({
        clientId: "owner",
        environmentId: scope.environmentId,
      ...
      const error = yield* broker
        .invoke<void>({
          scope: { ...scope, providerSessionId: "fresh-explicit-tab" },
          tabId,
          operation: "snapshot",
          input: {},
        })
