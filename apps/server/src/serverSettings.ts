...
function rejectLegacyProviderInstancesPatch(
  patch: ServerSettingsPatch,
): Effect.Effect<void, ServerSettingsUpdateConflictError> {
  return patch.providerInstances === undefined
    ? Effect.void
    : Effect.fail(
        new ServerSettingsUpdateConflictError({
          reason: "provider-instances-upgrade-required",
          detail:
            "This client cannot replace the provider instance map safely. Upgrade Pylon and retry through server.mutateProviderInstances.",
        }),
      );
}
...
const make = Effect.gen(function* () {
    ...
    Effect.sync(() => {
      ...
      for (const [sourceId, source] of Object.entries(next.usageLimitSources)) {
        ...
        changes.push({
          kind: "write",
          secretName,
          value: textEncoder.encode(managementKey),
        });
  ...
  const commitSettingsWithSecrets = (current: ServerSettings, candidate: ServerSettings) =>
    Effect.gen(function* () {
      const persisted = yield* persistProviderEnvironmentSecrets(current, candidate);
    ...
    compareAndSetPrimeAgentBinaryPath: (input) =>
      writeSemaphore.withPermits(1)(
        Effect.gen(function* () {
          ...
          const committed = yield* commitSettingsWithSecrets(
            current,
            applyServerSettingsPatch(current, patch),
          );
          return primeAgentBinaryBinding(committed.next, input.instanceId);
        }),
      ),
    mutateProviderInstances: (input) =>
      writeSemaphore.withPermits(1)(
        Effect.gen(function* () {
          ...
          const committed = yield* commitSettingsWithSecrets(current, candidate);
          const materialized = resolveTextGenerationProvider(committed.materialized);
          const receipt = {
            mutationId: input.mutationId,
            disposition: "applied" as const,
