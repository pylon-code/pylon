...
export interface PrimeAgentMaterializedIdentity {
  ...
  /** True only for an enabled participant in a set with more than one enabled Prime instance. */
  readonly nativeMultipleInstancesRequired: boolean;
...
export const materializePrimeAgentIdentities = Effect.fn("materializePrimeAgentIdentities")(
  ...
  > {
    ...
    const enabledInstanceCount = inputs.filter((input) => input.enabled).length;
    ...
    for (const input of inputs) {
      ...
      const identity = Object.freeze({
        ...
        nativeMultipleInstancesRequired: input.enabled && enabledInstanceCount > 1,
  ...
  const runtimeIdentity =
    immutableBackendIdentity.kind === "daemon" || identity.nativeOwnership === undefined
      ...
      : {
          ...
          nativeMultipleInstancesRequired: identity.nativeMultipleInstancesRequired,
        };
