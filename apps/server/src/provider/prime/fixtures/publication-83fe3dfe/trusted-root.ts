// @effect-diagnostics globalDate:off
import type { verifyPrimeSigstoreBundle } from "../../PrimeAgentDistributionVerifier.ts";

// Verified Sigstore TUF trust material captured with the immutable fixture on 2026-09-11.
export const trustedRoot = {
  mediaType: "application/vnd.dev.sigstore.trustedroot+json;version=0.1",
  tlogs: [
    {
      baseUrl: "https://rekor.sigstore.dev",
      hashAlgorithm: 1,
      publicKey: {
        rawBytes: Buffer.from(
          "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE2G2Y+2tabdTV5BcGiBIx0a9fAFwrkBbmLSGtks4L3qX6yYY0zufBnhC8Ur/iy55GhWP/9A/bY2LhC30M9+RYtw==",
          "base64",
        ),
        keyDetails: 5,
        validFor: { start: new Date("2021-01-12T11:53:27.000Z"), end: undefined },
      },
      logId: { keyId: Buffer.from("wNI9atQGlz+VWfO6LRygH4QUfY/8W4RFwiT5i5WRgB0=", "base64") },
      checkpointKeyId: undefined,
      operator: "",
    },
    {
      baseUrl: "https://log2025-1.rekor.sigstore.dev",
      hashAlgorithm: 1,
      publicKey: {
        rawBytes: Buffer.from(
          "MCowBQYDK2VwAyEAt8rlp1knGwjfbcXAYPYAkn0XiLz1x8O4t0YkEhie244=",
          "base64",
        ),
        keyDetails: 7,
        validFor: { start: new Date("2025-09-23T00:00:00.000Z"), end: undefined },
      },
      logId: { keyId: Buffer.from("zxGZFVvd0FEmjR8WrFwMdcAJ9vtaY/QXf44Y1wUeP6A=", "base64") },
      checkpointKeyId: undefined,
      operator: "",
    },
  ],
  certificateAuthorities: [
    {
      subject: { organization: "sigstore.dev", commonName: "sigstore" },
      uri: "https://fulcio.sigstore.dev",
      certChain: {
        certificates: [
          {
            rawBytes: Buffer.from(
              "",
              "base64",
            ),
          },
        ],
      },
      validFor: {
        start: new Date("2021-03-07T03:20:29.000Z"),
        end: new Date("2022-12-31T23:59:59.999Z"),
      },
      operator: "",
    },
    {
      subject: { organization: "sigstore.dev", commonName: "sigstore" },
      uri: "https://fulcio.sigstore.dev",
      certChain: {
        certificates: [
          {
            rawBytes: Buffer.from(
              "",
              "base64",
            ),
          },
          {
            rawBytes: Buffer.from(
              "",
              "base64",
            ),
          },
        ],
      },
      validFor: { start: new Date("2022-04-13T20:06:15.000Z"), end: undefined },
      operator: "",
    },
  ],
  ctlogs: [
    {
      baseUrl: "https://ctfe.sigstore.dev/test",
      hashAlgorithm: 1,
      publicKey: {
        rawBytes: Buffer.from(
          "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEbfwR+RJudXscgRBRpKX1XFDy3PyudDxz/SfnRi1fT8ekpfBd2O1uoz7jr3Z8nKzxA69EUQ+eFCFI3zeubPWU7w==",
          "base64",
        ),
        keyDetails: 5,
        validFor: {
          start: new Date("2021-03-14T00:00:00.000Z"),
          end: new Date("2022-10-31T23:59:59.999Z"),
        },
      },
      logId: { keyId: Buffer.from("CGCS8ChS/2hF0dFrJ4ScRWcYrBY9wzjSbea8IgY2b3I=", "base64") },
      checkpointKeyId: undefined,
      operator: "",
    },
    {
      baseUrl: "https://ctfe.sigstore.dev/2022",
      hashAlgorithm: 1,
      publicKey: {
        rawBytes: Buffer.from(
          "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEiPSlFi0CmFTfEjCUqF9HuCEcYXNKAaYalIJmBZ8yyezPjTqhxrKBpMnaocVtLJBI1eM3uXnQzQGAJdJ4gs9Fyw==",
          "base64",
        ),
        keyDetails: 5,
        validFor: { start: new Date("2022-10-20T00:00:00.000Z"), end: undefined },
      },
      logId: { keyId: Buffer.from("3T0wasbHETJjGR4cmWc3AqJKXrjePK3/h4pygC8p7o4=", "base64") },
      checkpointKeyId: undefined,
      operator: "",
    },
  ],
  timestampAuthorities: [
    {
      subject: { organization: "sigstore.dev", commonName: "sigstore-tsa-selfsigned" },
      uri: "https://timestamp.sigstore.dev/api/v1/timestamp",
      certChain: {
        certificates: [
          {
            rawBytes: Buffer.from(
              "",
              "base64",
            ),
          },
          {
            rawBytes: Buffer.from(
              "",
              "base64",
            ),
          },
        ],
      },
      validFor: { start: new Date("2025-07-04T00:00:00.000Z"), end: undefined },
      operator: "",
    },
  ],
} satisfies Parameters<typeof verifyPrimeSigstoreBundle>[1];
