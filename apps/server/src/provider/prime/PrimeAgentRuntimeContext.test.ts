  ...
  it.effect("materializes one frozen exact environment with explicit-home precedence", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        expect(identity.nativeMultipleInstancesRequired).toBe(false);
  ...
  it.effect("rejects every enabled participant in nested and symlink-aliased overlap groups", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const separateIdentity = readyIdentity(prepared, "prime-separate");
        expect(separateIdentity.effectiveHome).toBe(NodePath.join(canonicalRoot, "separate"));
        expect(separateIdentity.nativeMultipleInstancesRequired).toBe(true);
      }),
    ),
  );
});
