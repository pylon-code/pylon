 ...
 * release. At runtime the service refreshes it from Pylon's public releases
 * repository, so a new model can leave the legacy section without a product
 * release. Preference order is remote, then the on-disk copy of the last
 * successful fetch, then the bundle. A failed fetch never fails a provider
 * check.
 *
...
const ModelManifestSchema = Schema.Struct({
  ...
  currentModels: Schema.Record(
    ...
    Schema.Array(
      Schema.String.check(Schema.isNonEmpty(), Schema.isMaxLength(MODEL_MANIFEST_MAX_SLUG_LENGTH)),
    ).check(Schema.isMaxLength(MODEL_MANIFEST_MAX_MODELS_PER_PROVIDER)),
  ),
}).annotate({ parseOptions: { onExcessProperty: "error" } });
export type ModelManifestData = typeof ModelManifestSchema.Type;

const validateManifestDrivers = (manifest: ModelManifestData) => {
  const driverKinds = Object.keys(manifest.currentModels);
  return driverKinds.length <= MODEL_MANIFEST_MAX_PROVIDERS &&
    driverKinds.every((driverKind) => SUPPORTED_MANIFEST_DRIVER_KINDS.has(driverKind))
    ? Effect.succeed(manifest)
    : Effect.fail(
        new ModelManifestValidationError({ reason: "Unsupported model manifest driver kind" }),
      );
};

const decodeManifestJsonSchema = Schema.decodeUnknownEffect(
...
const decodeManifestCacheSchema = Schema.decodeUnknownEffect(
  Schema.fromJsonString(
    ManifestCacheFile as unknown as Schema.Codec<typeof ManifestCacheFile.Type>,
  ),
);
const decodeManifestCache = (input: unknown) =>
  decodeManifestCacheSchema(input).pipe(
    Effect.flatMap((cache) => validateManifestDrivers(cache.manifest).pipe(Effect.as(cache))),
  );
...
export const make = Effect.gen(function* () {
  ...
  const refresh = Effect.fn("ModelManifest.refresh")(function* () {
    ...
    const updateChecksEnabled = yield* settingsService.getSettings.pipe(
      Effect.map((settings) => settings.enableProviderUpdateChecks),
      Effect.orElseSucceed(() => false),
    );
    if (!updateChecksEnabled) return manifest;
    ...
    const fetched = yield* httpClient.get(MODEL_MANIFEST_URL).pipe(
      ...
      Effect.flatMap((response) =>
        collectUint8StreamText({
          ...
          maxBytes: MODEL_MANIFEST_MAX_BYTES,
        }),
      ),
      Effect.filterOrFail(
        (body) => !body.truncated && !body.invalidUtf8,
        () =>
          new ModelManifestValidationError({
