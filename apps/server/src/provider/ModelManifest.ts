...
const ModelManifestSchema = ModelManifestEnvelopeSchema.pipe(
  ...
  // The manifest is fetched from the network, so an unknown field is a reason to
  // distrust the payload rather than something to ignore. This has to sit on the
  // schema that actually decodes; it does not survive `Schema.check`.
).annotate({ parseOptions: { onExcessProperty: "error" } });
