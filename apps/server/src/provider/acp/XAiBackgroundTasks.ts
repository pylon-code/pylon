...
export function buildGrokBackgroundTaskEvents(input: {
  ...
  readonly completedTaskIds: Map<string, string | undefined>;
  readonly ambiguousTaskIds: Set<string>;
  ...
  const { tasks, completedTaskIds, ambiguousTaskIds, toolCallId, toolCallStatus, turnId } = input;
  ...
  const start = (
    ...
    if (ambiguousTaskIds.has(id)) return undefined;
    const known = tasks.get(id);
    if (known) {
      if (!toolUseId || known.payload.toolUseId === toolUseId) return known;
      // Polls identify only the provider ID. Once two live tool calls reuse it,
      // their later results cannot be attributed safely to either activation.
      tasks.delete(id);
      completedTaskIds.set(id, known.payload.toolUseId);
      ambiguousTaskIds.add(id);
      events.push({
        type: "task.completed",
        payload: { ...known.payload, status: "stopped" },
        ...attribution(known),
      });
      return undefined;
    }
    const previousToolUseId = completedTaskIds.get(id);
    if (completedTaskIds.has(id) && (!toolUseId || previousToolUseId === toolUseId)) {
      return undefined;
    }
    const task: GrokBackgroundTaskRecord = {
      rawTaskId: id,
      payload: {
        taskId: RuntimeTaskId.make(completedTaskIds.has(id) ? `${id}#${toolUseId}` : id),
        taskType,
  ...
  const complete = (
    ...
    tasks.delete(task.rawTaskId);
    completedTaskIds.delete(task.rawTaskId);
    completedTaskIds.set(task.rawTaskId, task.payload.toolUseId);
    events.push({
      type: "task.completed",
