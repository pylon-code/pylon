...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no Pylon runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when Pylon is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the Pylon referrer through Grok OAuth env", () => {
