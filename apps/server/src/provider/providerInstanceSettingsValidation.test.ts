...
describe("provider multiple-instance settings validation", () => {
  it("is explicit for built-ins, fail-closed for unknown drivers, and zero-work on native Windows", () => {
    ...
    expect(getProviderMultipleInstanceSupport("primeAgent", "linux")).toMatchObject({
      supported: false,
      reason: expect.stringContaining("did not pass A/B isolation"),
    });
  ...
  it.effect("validates four independent Prime homes before applying the artifact blocker", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const blocked = yield* validateProviderInstanceSettings({
          settings: settingsWithInstances(providerInstances),
          platform: "darwin",
          hostEnvironment: { HOME: root },
        }).pipe(Effect.flip);
        expect(blocked.detail).toContain("did not pass A/B isolation");
      }),
