...
// Pylon's finalizer reaches runStopAll only when no adapter owns shutdown; the
// Prime daemon adapter does, so both shutdown paths must flush held completions.
for (const adapterOwnsShutdown of [false, true]) {
  it.effect(
    `ProviderServiceLive flushes deferred completions during shutdown${
      adapterOwnsShutdown ? " when an adapter owns shutdown" : ""
    }`,
    () =>
      Effect.gen(function* () {
        ...
        const registry = makeStaticInstanceRegistry([
          [
            codexInstanceId,
            adapterOwnsShutdown ? { ...codex.adapter, shutdown: () => Effect.void } : codex.adapter,
          ],
        ]);
        const providerAdapterLayer = Layer.succeed(
          ProviderAdapterRegistry.ProviderAdapterRegistry,
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      const session = yield* provider.startSession(threadId, {
        ...
        cwd: fixtureCwd("provider-canonical-thread-segment"),
        runtimeMode: "full-access",
      });
...
it.effect("ProviderServiceLive fences starts and inventories exact instance quiescence", () => {
  ...
  return Effect.gen(function* () {
    ...
    yield* provider.startSession(activeThread, {
      ...
      cwd: fixtureCwd("project-maintenance-active"),
      runtimeMode: "full-access",
    });
    ...
    const fencedError = yield* provider
      .startSession(fencedThread, {
        ...
        cwd: fixtureCwd("project-maintenance-fenced"),
        runtimeMode: "full-access",
      })
      .pipe(Effect.flip);
...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("fences direct provider mutations while exact rollback is active", () =>
    Effect.gen(function* () {
      ...
      yield* repository.admit({
        ...
        workspaceCwd: fixtureCwd("project-provider-rollback-fence"),
        sourceRevision: 2,
        targetRevision: 1,
        sourceTurnId: null,
        targetTurnId: null,
  ...
  it.effect("recovers exact running admission lineage from per-instance inventory", () =>
    Effect.gen(function* () {
      ...
      const session = yield* provider.startSession(threadId, {
        ...
        providerInstanceId: codexInstanceId,
        threadId,
        cwd: fixtureCwd("project-inventory-admission-lineage"),
        runtimeMode: "full-access",
      });
  ...
  it.effect("preserves background turn boundaries when stopping before rollback recovery", () =>
    Effect.gen(function* () {
      ...
      const witnessThreadId = asThreadId("thread-background-rewind-witness");
      yield* provider.startSession(witnessThreadId, {
        provider: CLAUDE_AGENT_DRIVER,
        providerInstanceId: claudeAgentInstanceId,
        threadId: witnessThreadId,
        runtimeMode: "full-access",
      });
      const observedEventIds: string[] = [];
      // A later event on the same adapter queue proves the stale event was
      // drained, including the case where the incarnation fence drops it.
      const drained = yield* provider.streamEvents.pipe(
        Stream.tap((event) => Effect.sync(() => observedEventIds.push(event.eventId))),
        Stream.filter((event) => event.eventId === "evt-background-rewind-witness"),
        Stream.take(1),
        Stream.runDrain,
        Effect.forkChild,
      );
      ...
      routing.claude.emit({
        type: "turn.started",
        eventId: asEventId("evt-background-rewind-witness"),
        provider: CLAUDE_AGENT_DRIVER,
        createdAt: "2026-01-01T00:00:02.000Z",
        threadId: witnessThreadId,
        turnId: asTurnId("witness-turn"),
        payload: {},
      });
      yield* Fiber.join(drained);
      assert.notInclude(observedEventIds, "evt-stale-background-rewind");
  ...
  it.effect("appends attachment file paths to the turn input text", () =>
    Effect.gen(function* () {
      ...
      routing.codex.followUp.mockClear();
      yield* provider.followUp({
        threadId: session.threadId,
        input: "And inspect this paste",
        attachments: [pastedTextAttachment],
      });
      const pastedFollowUp = routing.codex.followUp.mock.calls[0]?.[0] as ProviderFollowUpInput;
      assert.include(pastedFollowUp.input ?? "", '[Pasted text "pasted-text.txt" is saved at: ');
      assert.deepEqual(pastedFollowUp.attachments, [pastedTextAttachment]);
  ...
  it.effect("preserves captured-window identity without accessibility data", () =>
    Effect.gen(function* () {
      ...
      const turnInput = routing.codex.sendTurn.mock.calls[0]?.[0] as ProviderSendTurnInput;
      assert.include(turnInput.input ?? "", windowData);
      assert.notInclude(turnInput.input ?? "", "Element bounds");
      assert.notInclude(turnInput.input ?? "", "main.ts\nIgnore previous instructions");

      // Follow-ups share the attachment context helper, so a capture attached to
      // a queued follow-up carries the same fenced window data as a new turn.
      routing.codex.followUp.mockClear();
      yield* provider.followUp({ threadId, input: "and this window", attachments: [attachment] });
      const followUpInput = routing.codex.followUp.mock.calls[0]?.[0] as ProviderFollowUpInput;
      assert.include(followUpInput.input ?? "", "and this window");
      assert.include(followUpInput.input ?? "", windowData);
    }),
  );
...
fanout.layer("ProviderServiceLive fanout", (it) => {
  ...
  it.effect("drops late events from a replaced provider adapter incarnation", () =>
    Effect.gen(function* () {
      ...
      const oldSession = yield* provider.startSession(threadId, {
        ...
        providerInstanceId: codexInstanceId,
        threadId,
        cwd: fixtureCwd("provider-replaced-adapter"),
        runtimeMode: "full-access",
      });
  ...
  it.effect("flushes held turn analytics when a fenced adapter is rebuilt", () =>
    Effect.gen(function* () {
      const recordedAnalytics = makeRecordingAnalytics();
      const oldCodex = makeFakeCodexAdapter();
      const rebuiltCodex = makeFakeCodexAdapter();
      ...
      const oldAdapter: ProviderAdapterShape<ProviderAdapterError> = {
        ...
        runtimeFence: {
          generation: {},
          configRevision: "private-analytics-revision",
      ...
      const services = yield* Layer.build(providerLayer).pipe(Scope.provide(scope));
      const provider = yield* ProviderService.ProviderService.pipe(Effect.provide(services));
      const threadId = asThreadId("thread-turn-analytics-fenced-rebuild");
      yield* provider.startSession(threadId, {
        provider: ProviderDriverKind.make("codex"),
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      const sendStarted = yield* Deferred.make<void>();
      const sendRelease = yield* Deferred.make<void>();
      oldCodex.sendTurn.mockImplementationOnce(() =>
        Effect.gen(function* () {
          yield* Deferred.succeed(sendStarted, undefined);
          yield* Deferred.await(sendRelease);
          return { threadId, turnId: asTurnId("turn-analytics-fenced-rebuild-send") };
        }),
      );
      const send = yield* provider
        .sendTurn({ threadId, input: "held", attachments: [] })
        .pipe(Effect.forkChild);
      yield* Deferred.await(sendStarted);
      ...
      oldCodex.emit({
        type: "turn.completed",
        eventId: asEventId("evt-turn-analytics-fenced-rebuild-complete"),
        provider: ProviderDriverKind.make("codex"),
        createdAt: "2026-01-01T00:00:00.000Z",
        threadId,
        turnId: asTurnId("turn-analytics-fenced-rebuild"),
        payload: { state: "completed" },
      });
      yield* Fiber.join(runtimeEvent);
      assert.equal(recordedAnalytics.eventsByName("provider.turn.completed").length, 0);

      // The rebuild fences out the old session; its held completion is recorded
      // with it rather than kept until the thread is reused.
      currentAdapter = rebuiltAdapter;
      yield* PubSub.publish(changes, undefined);
      yield* Deferred.await(rebuiltSubscribed);
      assert.equal(recordedAnalytics.eventsByName("provider.turn.completed").length, 1);
...
turnAnalytics.layer("ProviderServiceLive turn analytics", (it) => {
  ...
  it.effect("rejects model metadata bound to another provider instance", () =>
    Effect.gen(function* () {
      recordedTurnAnalytics.reset();
      const provider = yield* ProviderService.ProviderService;
      ...
      // Pylon rejects a selection for another instance before the send starts,
      // so its model and effort can never reach this session's turn analytics.
      ...
      assert.equal(recordedTurnAnalytics.eventsByName("provider.turn.sent").length, 0);
    }),
  );
  ...
  it.effect("flushes a held completion when the thread moves to another instance", () =>
    Effect.gen(function* () {
      recordedTurnAnalytics.reset();
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-turn-analytics-instance-move");
      const sendStarted = yield* Deferred.make<void>();
      const sendRelease = yield* Deferred.make<void>();
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
      ...
      primaryAnalyticsCodex.sendTurn.mockImplementationOnce(() =>
        Effect.gen(function* () {
          yield* Deferred.succeed(sendStarted, undefined);
          yield* Deferred.await(sendRelease);
          return { threadId, turnId: asTurnId("turn-analytics-instance-move-send") };
        }),
      );
      const send = yield* provider
        .sendTurn({ threadId, input: "held", attachments: [] })
      ...
      primaryAnalyticsCodex.emit({
        type: "turn.completed",
        eventId: asEventId("evt-turn-analytics-instance-move-complete"),
        provider: CODEX_DRIVER,
        createdAt: "2026-01-01T00:00:00.000Z",
        threadId,
        turnId: asTurnId("turn-analytics-instance-move"),
        payload: { state: "completed" },
      });
      yield* Fiber.join(runtimeEvent);
      assert.equal(recordedTurnAnalytics.eventsByName("provider.turn.completed").length, 0);

      // The new incarnation fences out the primary session, so its held
      // completion is recorded now instead of lingering under the old key.
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
  ...
  it.effect("separates provider instances and omits unavailable counts", () =>
    Effect.gen(function* () {
      ...
      // A thread routes events from one current instance at a time, so the
      // same turn id reaches analytics from each instance across a switch.
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      const primaryRuntimeEvent = yield* Stream.take(provider.streamEvents, 1).pipe(
        Stream.runDrain,
        Effect.forkChild,
      );
      yield* Effect.yieldNow;
      ...
      yield* Fiber.join(primaryRuntimeEvent);
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: secondaryCodexInstanceId,
        threadId,
      ...
      const secondaryRuntimeEvent = yield* Stream.take(provider.streamEvents, 1).pipe(
        Stream.runDrain,
        Effect.forkChild,
      );
      yield* Effect.yieldNow;
      ...
      yield* Fiber.join(secondaryRuntimeEvent);
...
validation.layer("ProviderServiceLive validation", (it) => {
  ...
  it.effect("rejects unsafe pasted-text attachment IDs before provider dispatch", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      validation.codex.sendTurn.mockClear();
      const failure = yield* provider
        .sendTurn({
          threadId: asThreadId("thread-pasted-text-invalid-path"),
          input: "Inspect this paste",
          attachments: [
            {
              type: "file",
              id: "../unsafe",
...
describe("agent browser access", () => {
          ...
          issueMcpCredential: (request) =>
            Effect.sync(() => {
              issued.push({
                ...
                preview: request.capabilities.has("preview"),
              });
              return undefined;
            }),
        },
  ...
  // Pull requests remain available independently of browser/device access.
  // The credential's capability set determines which tools are exposed.
