...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  ...
  it.effect(
    "retries incomplete project skill discovery before using Cursor's native slash form",
    () =>
      Effect.gen(function* () {
        const adapter = yield* CursorAdapter;
        const settings = yield* ServerSettingsService;
        ...
        const blockedRoot = NodePath.join(workspace, ".agents", "skills");
        yield* Effect.promise(() =>
          NodeFSP.mkdir(NodePath.dirname(blockedRoot), { recursive: true }),
        );
        yield* Effect.promise(() => NodeFSP.writeFile(blockedRoot, "not a directory", "utf8"));
        yield* adapter.sendTurn({ threadId, input: "please $review this", attachments: [] });
        yield* Effect.promise(() => NodeFSP.rm(blockedRoot));
        yield* adapter.sendTurn({
          threadId,
