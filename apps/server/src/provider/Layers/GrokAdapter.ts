...
interface GrokSessionContext {
  ...
  stopped: boolean;
  /** Live monitor and shell identities, with their originating turns. */
  readonly backgroundTasks: Map<string, GrokBackgroundTaskRecord>;
  readonly completedBackgroundTaskIds: Map<string, string | undefined>;
  readonly ambiguousBackgroundTaskIds: Set<string>;
}
...
export function makeGrokAdapter(grokSettings: GrokSettings, options?: GrokAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const settlePromptInFlight = (
      ...
      ownerSessionIncarnationId: ProviderSession["sessionIncarnationId"],
      options?: {
        ...
        if (!settlementBelongsToLiveContext) {
          ...
          if (options?.emitTurnCompletion !== false) {
            if (options?.errorMessage !== undefined) {
              yield* offerRuntimeEvent(ownerSessionIncarnationId, {
                type: "turn.completed",
    ...
    /** Surface Grok plan.md as Pylon's proposed-plan card (while writing + on exit). */
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const started = yield* Effect.gen(function* () {
            ...
            // Capture plan into Pylon's proposed-plan UI and abandon the native gate so
          ...
          const ctx: GrokSessionContext = {
            ...
            completedBackgroundTaskIds: new Map(),
            ambiguousBackgroundTaskIds: new Set(),
          };
              ...
              Effect.gen(function* () {
                ...
                if (
                  ...
                  for (const taskEvent of buildGrokBackgroundTaskEvents({
                    ...
                    completedTaskIds: ctx.completedBackgroundTaskIds,
                    ambiguousTaskIds: ctx.ambiguousBackgroundTaskIds,
    ...
    return {
      ...
      capabilities: {
        ...
        conversationRollback: BUILT_IN_ADAPTER_CONVERSATION_ROLLBACK_MODES.grok,
      },
