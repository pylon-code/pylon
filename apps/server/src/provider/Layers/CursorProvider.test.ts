...
const cursorAcpDiscoveryFailedMessage = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart Pylon, and try again.",
...
const cursorCliCommandMissingMessage = [
  ...
  `Install or enable the Cursor CLI, make sure \`${missingCursorBinaryPath}\` is on PATH, then restart Pylon.`,
  "See https://cursor.com/docs/cli/installation.",
].join(" ");
...
describe("Cursor skills", () => {
  ...
  it("rewrites Unicode currency skill aliases but leaves monetary amounts intact", () => {
    ...
    for (const symbol of ["€", "£", "¥", "₹", "₩", "₿", "𑿝"]) {
      ...
      expect(rewriteCursorSkillMentions(`5${symbol}review ${symbol}unknown`, names)).toBe(
        `5${symbol}review ${symbol}unknown`,
      );
    }
  });
});
