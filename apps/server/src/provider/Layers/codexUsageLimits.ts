...
import { clampPercent, makeUsageLimits } from "../usageLimitsSnapshot.ts";
...
function kindForDuration(mins: number): NonNullable<ServerProviderUsageWindow["kind"]> {
  if (mins >= MONTH_MINS) return "monthly";
  if (mins >= WEEK_MINS) return "weekly";
  return "session";
}

function labelForKind(kind: NonNullable<ServerProviderUsageWindow["kind"]>): string {
...
export function codexRateLimitsToLimits(input: {
  ...
  return {
    ...
    ...(resetCredits ? { resetCredits: { ...resetCredits, checkedAt: input.checkedAt } } : {}),
  };
}
