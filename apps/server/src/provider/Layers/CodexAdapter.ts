          ...
          ...(mcpSession
            ? {
                ...
                appServerArgs: [
                  ...
                  // This overrides the feature for this app-server process only; the user's config.toml is untouched.
        ...
        const eventFiber = yield* Stream.runForEach(runtime.events, (event) =>
          Effect.gen(function* () {
            ...
            const mappedEvents = mapToRuntimeEvents(event, event.threadId).map((runtimeEvent) => {
              ...
              if (stampedRuntimeEvent.type === "turn.completed" && stampedRuntimeEvent.turnId) {
                return {
                  ...stampedRuntimeEvent,
                  payload: {
                    ...stampedRuntimeEvent.payload,
                    ...(usageLimitMessage ? { errorMessage: usageLimitMessage } : {}),
                    tokenUsage: completeCodexTurnTokenUsage(
                      turnTokenUsage,
                      String(stampedRuntimeEvent.turnId),
                      stampedRuntimeEvent.payload.state === "completed",
                    ),
                  },
                } satisfies ProviderRuntimeEvent;
              }
              if (stampedRuntimeEvent.type === "turn.aborted" && stampedRuntimeEvent.turnId) {
                return {
                  ...stampedRuntimeEvent,
                  payload: {
                    ...stampedRuntimeEvent.payload,
