...
const httpClientLayer = (handler: (url: string) => Response) =>
  Layer.succeed(
...
describe("ModelManifest service", () => {
  it.live("fetches the manifest only from Pylon's public releases repository", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        serviceLayers({
          prefix: "model-manifest-pylon-endpoint-test",
          response: (url) => {
            fetchedUrl = url;
  ...
  it.live("rejects a schema-valid response beyond the byte limit", () =>
    Effect.gen(function* () {
      ...
      Effect.provide(
        serviceLayers({
          prefix: "model-manifest-oversize-test",
          response: () =>
            Response.json({
              version: 1,
              currentModels: {
                codex: Array.from({ length: 200 }, (_, index) => `${index}-${"x".repeat(200)}`),
              },
            }),
        }),
      ),
  ...
  it.live("does not fetch when settings cannot confirm update checks are enabled", () => {
    let fetchCount = 0;
    return Effect.gen(function* () {
