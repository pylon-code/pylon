...
it.effect(
  "queued configuration patches derive omitted settings after acquiring the lifecycle lock",
  () =>
    Effect.gen(function* () {
      ...
      const blocker = yield* service
        .withLifecycleLock(
          Deferred.succeed(locked, undefined).pipe(Effect.andThen(Deferred.await(release))),
        )
        .pipe(Effect.forkChild);
      yield* Deferred.await(locked);
      const disabling = yield* service.configure({ enabled: false }).pipe(Effect.forkChild);
      // Let both calls queue behind the held lifecycle permit before releasing it.
      yield* Effect.yieldNow;
      const updatingPermission = yield* service
        .configure({ agentAccessEnabled: false })
        .pipe(Effect.forkChild);
      yield* Effect.yieldNow;
      yield* Deferred.succeed(release, undefined);
      yield* Fiber.join(blocker);
      yield* Fiber.join(disabling);
      const state = yield* Fiber.join(updatingPermission);
      expect(state.hostStatus).toBe("disabled");
      expect(state.agentAccessEnabled).toBe(false);
      expect(state.devices).toEqual([]);
      expect((yield* Ref.get(settings)).enableDeviceSupport).toBe(false);
    }).pipe(Effect.scoped),
);
