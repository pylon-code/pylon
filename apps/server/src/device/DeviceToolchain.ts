...
const ensureTool = Effect.fn("DeviceToolchain.ensureTool")(function* (
  ...
  const path = yield* Path.Path;
  const fs = yield* FileSystem.FileSystem;
  const paths = select(deviceToolchainPaths(path, baseDir));
  const fail = (step: string, cause: unknown) =>
    new DeviceToolchainInstallError({ tool: spec.name, step, cause });
  yield* fs
    .makeDirectory(path.dirname(paths.installDir), { recursive: true })
    .pipe(Effect.mapError((cause) => fail("preparing the install directory", cause)));
  const compromised = yield* Deferred.make<never, DeviceToolchainInstallError>();
  // Installation is shared by independent environment server processes. Hold a
  // filesystem lease across checking/removing the incomplete tree and atomic
  // publication. A lost lease interrupts npm and staging before any publish.
  const lockedInstall = Effect.acquireUseRelease(
    Effect.tryPromise({
      try: () =>
        lockfile.lock(paths.installDir, {
          realpath: false,
          stale: 60_000,
          update: 10_000,
          retries: 0,
          onCompromised: (cause) =>
            Deferred.doneUnsafe(compromised, Effect.fail(fail("holding the install lock", cause))),
        }),
      catch: (cause) => fail("acquiring the install lock", cause),
    }),
    () => Effect.raceFirst(installTool(spec, paths), Deferred.await(compromised)),
    (release) => Effect.tryPromise(() => release()).pipe(Effect.ignore),
  ).pipe(
    Effect.retry({
      while: (error) =>
        error.step === "acquiring the install lock" &&
        typeof error.cause === "object" &&
        error.cause !== null &&
        "code" in error.cause &&
        error.cause.code === "ELOCKED",
      schedule: Schedule.spaced("250 millis").pipe(Schedule.upTo({ duration: "10 minutes" })),
    }),
  );
  return yield* installLock.withPermit(lockedInstall);
});
