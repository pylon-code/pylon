...
it.effect("keeps PowerShell metacharacters inside the Explorer path literal", () =>
  Effect.sync(() => {
    const target = "C:\\workspace\\author's\n$(Start-Process calc).ts";
    assert.equal(
      ExternalLauncher.buildFileExplorerRevealPowerShellSource("explorer.exe", target),
      "$ProgressPreference = 'SilentlyContinue'; Start-Process 'explorer.exe' -ArgumentList ('/select,\"' + 'C:\\workspace\\author''s\n$(Start-Process calc).ts' + '\"')",
    );
  }),
);
...
it.effect.skipIf(windowsHost)(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.equal(probe.options.forceKillAfter, "1 second");
      assert.equal(probe.options.extendEnv, true);
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
...
it.effect("bounds output from a noisy Linux directory handler probe", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
    ...
    const editors = yield* Effect.gen(function* () {
      ...
      Effect.provide(
        testLayer({
          ...
          env: { PATH: binDir, DISPLAY: ":0" },
          spawnResult: (command) =>
            command.command === "xdg-mime" ? { stdout: "x".repeat(2048) } : undefined,
        }),
      ),
    );
...
it.live.skipIf(windowsHost)("force-kills a real Linux handler probe that ignores termination", () =>
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-editors-" });
    yield* fileSystem.writeFileString(path.join(binDir, "xdg-open"), "#!/bin/sh\n");
    yield* fileSystem.chmod(path.join(binDir, "xdg-open"), 0o755);
    yield* fileSystem.writeFileString(
      path.join(binDir, "xdg-mime"),
      "#!/bin/sh\ntrap '' TERM\nwhile :; do /bin/sleep 1; done\n",
    );
    yield* fileSystem.chmod(path.join(binDir, "xdg-mime"), 0o755);
    ...
    const editors = yield* Effect.gen(function* () {
      ...
      Effect.provide(
        Layer.mergeAll(
          ExternalLauncher.layer.pipe(Layer.provide(NodeServices.layer)),
          Layer.succeed(HostProcessPlatform, "linux"),
          Layer.succeed(SpawnExecutableResolution, (command) => command),
          ConfigProvider.layer(ConfigProvider.fromEnv({ env: { PATH: binDir, DISPLAY: ":0" } })),
        ),
      ),
    );
    ...
    assert.isBelow(finishedAt - startedAt, 4_500);
  }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
