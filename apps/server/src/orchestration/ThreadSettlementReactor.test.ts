...
const makeHarness = Effect.fn("makeThreadSettlementHarness")(function* (options: HarnessOptions) {
  ...
  const serverSettings = ServerSettingsService.of({
    ...
    updateSettings,
    // Pylon-only member of ServerSettingsService; this reactor never mutates
    // provider instances, so the stub only needs to satisfy the shape.
    mutateProviderInstances: () => Effect.die(new Error("unexpected provider mutation")),
