...
describe("ProviderRuntimeIngestion", () => {
  ...
  async function createHarness(options?: {
    ...
    // Advance pacing deterministically while keeping real sleeps and deadlines.
    let clockOffsetMs = 0;
    ...
    const runEffect = <A, E>(effect: Effect.Effect<A, E>) => testRuntime.runPromise(effect);
  ...
  it("preserves answered questions and leaves newer, child and async questions pending", async () => {
    ...
    // Preserve an explicitly stored async question when a different native
    // request completes.
    await harness.dispatch({
      type: "thread.activity.append",
