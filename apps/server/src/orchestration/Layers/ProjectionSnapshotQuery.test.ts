...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  ...
  it.effect("counts every other Pylon user message for first-turn title eligibility", () =>
    Effect.gen(function* () {
      const query = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      ...
      for (const { text, attachments, hasOtherUserMessages } of [
        { text: "/compact", attachments: null, hasOtherUserMessages: true },
        {
          text: "\t\n\r /CoMpAcT\u00a0\u2028\ufeff",
          attachments: "[ ]",
  ...
  it.effect("keeps archived threads out of the main shell snapshot", () =>
    Effect.gen(function* () {
      ...
      assert.equal(archivedShellSnapshot.threads[0]?.activeOrderKey, "hq");
