...
it.layer(NodeServices.layer)("message context plumbing", (it) => {
  it.effect("persists native follow-up context on the active turn", () =>
    Effect.gen(function* () {
      const readModel = makeReadModel();
      const activeTurnId = TurnId.make("active-turn");
      const active: OrchestrationReadModel = {
        ...readModel,
      ...
      const result = yield* decideOrchestrationCommand({
        command: {
          ...
          commandId: CommandId.make("follow-up-context"),
          threadId: ThreadId.make("thread-1"),
          message: {
            messageId: MessageId.make("follow-up-context"),
            role: "user",
      ...
      expect(sent).toMatchObject({ payload: { turnId: activeTurnId, context } });
      ...
      expect(projected.threads[0]?.messages[0]?.context).toEqual(context);
    }),
  );
