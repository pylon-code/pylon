...
describe("normalizeDispatchCommand attachments", () => {
  ...
  it.effect("rebinds native follow-up context records through client command decoding", () =>
    Effect.gen(function* () {
      const normalized = yield* normalizeDispatchCommand(
      ...
      if (normalized.type !== "thread.input-queue.follow-up") {
        throw new Error("Expected a thread.input-queue.follow-up command.");
      }
      const persistedId = normalized.message.attachments[0]!.id;
      expect(persistedId.startsWith("thread-1-")).toBe(true);
      const records = normalized.message.context?.records ?? [];
