      ...
      if (input.directoryPath !== undefined) {
        ...
        const entries = yield* Effect.tryPromise({
          try: async () => {
            ...
            const eligible = children
              .filter(
                (child) =>
                  ...
                  (input.directoryCursor === undefined || child.name > input.directoryCursor),
              )
              .sort((left, right) =>
                left.name < right.name ? -1 : left.name > right.name ? 1 : 0,
              );
            const hasMore = eligible.length > DIRECTORY_PAGE_SIZE;
            const page = eligible.slice(0, DIRECTORY_PAGE_SIZE);
            ...
            return { entries, nextDirectoryCursor: hasMore ? page.at(-1)?.name : undefined };
          },
          catch: toError,
        });
        // Classify ignored entries without letting git availability decide whether a file exists.
        const ignored = new Set<string>();
        for (let offset = 0; offset < entries.entries.length; offset += 1000) {
          const chunk = entries.entries.slice(offset, offset + 1000);
        ...
        return {
          ...
          truncated: entries.nextDirectoryCursor !== undefined,
          directoryPath,
          ...(entries.nextDirectoryCursor === undefined
            ? {}
            : { nextDirectoryCursor: entries.nextDirectoryCursor }),
        };
      }
      return yield* Effect.gen(function* () {
        const searchIndex = yield* WorkspaceSearchIndex.WorkspaceSearchIndex;
