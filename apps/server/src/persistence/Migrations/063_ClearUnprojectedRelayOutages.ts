...
export default Effect.gen(function* () {
  ...
  yield* sql`
    ...
    WHERE outage.kind = 'task.progress'
      ...
      AND NOT EXISTS (
        SELECT 1 FROM projection_thread_activities AS worker
        WHERE worker.thread_id = outage.thread_id
          AND worker.kind LIKE 'task.%'
          AND worker.activity_id NOT LIKE 'relay-observer-unavailable:%'
          AND CASE WHEN json_valid(worker.payload_json)
            THEN json_extract(worker.payload_json, '$.taskId') END
