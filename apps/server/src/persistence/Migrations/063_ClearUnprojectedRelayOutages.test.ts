    ...
    it.effect("drops outages for workers a thread never started and keeps the rest", () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          ...
          [
            // A worker with any projected activity keeps its outage, even if
            // an older provider never emitted its task.started row.
            "relay-observer-unavailable:job-historical:1",
