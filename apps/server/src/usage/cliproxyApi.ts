...
import { usageLimitsFromClaudeOAuthResponse } from "../provider/claudeOAuthUsage.ts";
import { normalizeUsageWindow } from "@t3tools/shared/usageLimits";
import { makeUsageLimits } from "../provider/usageLimitsSnapshot.ts";
  ...
  const readAccount = Effect.fn("CliproxyApi.readAccount")(function* (
    ...
    const read = Effect.gen(function* () {
      if (account.provider === "claude") {
        ...
        const limits =
          usageLimitsFromClaudeOAuthResponse(usage, checkedAt, "cliproxyClaudeOAuth") ??
          makeUsageLimits({ checkedAt, windows: [], source: "cliproxyClaudeOAuth" });
        return {
          ...base,
          plan: "Claude Subscription",
          usageLimits: {
            ...limits,
            windows: limits.windows.map((window) => normalizeUsageWindow(window, "claudeAgent")),
          },
        };
      }
