...
export const make = Effect.gen(function* () {
  ...
  const consumeResetCredit = (input: UsageLimitSourceConsumeResetCreditInput) =>
    Effect.gen(function* () {
      ...
      yield* publish([...previous.filter((source) => source.id !== input.sourceId), snapshot]);
      const refreshFailed =
        "error" in snapshot ||
        snapshot.accounts.some(
          (account) =>
            account.id === input.accountId &&
            account.usageLimits.unavailable?.reason === "probeFailed",
        );
      return refreshFailed
        ? {
            ...
            warning: [
              result.warning,
              "The hub answered the reset request, but its updated limits could not be read. Refresh to check.",
            ]
              .filter(Boolean)
              .join(" "),
          }
  ...
  yield* Effect.forever(
    interval.pipe(
      Effect.flatMap((wait) => {
        const enabled = Duration.toMillis(Duration.fromInputUnsafe(wait)) > 0;
        return Effect.sleep(enabled ? wait : "60 seconds").pipe(
          Effect.andThen(
            enabled
