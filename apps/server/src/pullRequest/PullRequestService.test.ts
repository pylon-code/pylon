...
it.effect("does not reuse a stats response that finishes after invalidation", () =>
  Effect.gen(function* () {
    ...
    const service = yield* makeService({
      ...
      providers: [
        fakeProvider("github", {
          listChangeRequestStats: (input) =>
            Effect.gen(function* () {
              const additions = ++reads;
              if (additions === 1) {
                yield* Deferred.succeed(started, undefined);
                yield* Deferred.await(release);
              }
              return input.changeRequests.map((ref) => ({ ...ref, additions, deletions: 0 }));
            }),
        }),
      ],
    });
    const pending = yield* service.listStats({ refs: [reference] }).pipe(Effect.forkChild());
    yield* Deferred.await(started);
    yield* service.invalidate({ reference });
    assert.strictEqual((yield* service.listStats({ refs: [reference] })).stats[0]?.additions, 2);
