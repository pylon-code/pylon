...
<header>↑ Drag Pylon into the list above</header>
<button id="app" draggable="true" aria-label="Drag Pylon to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Pylon</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Pylon icon is missing.");
