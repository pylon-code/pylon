...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, "Name=Pylon (Nightly)");
    ...
    assert.include(entry, "MimeType=x-scheme-handler/pylon-code;");
  });
  ...
  it("carries structured context on registration errors", () => {
    ...
    assert.equal(
      writeError.message,
      "Failed to register the pylon-code:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
    ...
    assert.equal(
      exitError.message,
      "Failed to register the pylon-code:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
