...
let paths: { bundle: string; dataHome: string; appId: string };
...
it.each([true, false])("uses the active config syntax and user bindings (Lua: %s)", async (lua) => {
  ...
  const result = await hyprlandCaptureShortcut("com.pylon.code", directory);
  expect(result.shortcutConfigPath).toBe(
    NodePath.join(config, lua ? "bindings.lua" : "bindings.conf"),
  );
  expect(result.shortcutBinding).toBe(
    lua
      ? 'hl.bind("CTRL + SHIFT + 2", hl.dsp.global("com.pylon.code:capture-window"))'
      : "bind = CTRL SHIFT, 2, global, com.pylon.code:capture-window",
  );
