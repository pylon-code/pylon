...
export type KdeCapturePaths = {
  ...
  /** Channel app ID, so stable, nightly, and dev installs never replace each other's helper. */
  readonly appId: string;
};

export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, paths.appId, "kde-capture", KDE_CAPTURE_EXECUTABLE),
    desktop: NodePath.join(paths.dataHome, "applications", `${paths.appId}.KdeCapture.desktop`),
  };
}
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall Pylon.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall Pylon.",
        );
