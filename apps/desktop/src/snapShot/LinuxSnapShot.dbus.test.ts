...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.PylonSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("com.pylon.code")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.pylon.code")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.pylon.code")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    ...
    expect(await updated.backend("com.pylon.code")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.pylon.code", {
      flash: true,
      animate: true,
    });
    ...
    const kde = connect();
    expect(await kde.backend("com.pylon.code")).toBe("kde");
    ...
    const command = [
      ...
      "--session",
      "--dest",
      "com.pylon.code.NiriTest.SnapShot",
      "--object-path",
      "/com/pylon/code/SnapShot",
      "--method",
      "com.pylon.code.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "com.pylon.code.NiriTest.SnapShot",
        path: "/com/pylon/code/SnapShot",
        interface: "com.pylon.code.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
