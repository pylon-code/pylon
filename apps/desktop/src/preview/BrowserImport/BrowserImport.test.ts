 ...
 */
const withImporter = Effect.fnUntraced(function* (overrideFileSystem?: FileSystem.FileSystem) {
  ...
  const importer = yield* BrowserImport.BrowserImport.pipe(
    Effect.provide(
      BrowserImport.layer.pipe(
        ...
        Layer.provide(Layer.succeed(HostProcessExecutablePath, "/Applications/Pylon.app")),
        Layer.provide(Layer.succeed(FileSystem.FileSystem, overrideFileSystem ?? fileSystem)),
...
describe("BrowserImport.listSources", () => {
  it.effect("reports Full Disk Access when only a named Safari profile exists", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const opened: string[] = [];
      const { importer, home } = yield* withImporter({
        ...fileSystem,
        open: (path, options) => {
          if (!path.endsWith("Cookies.binarycookies")) return fileSystem.open(path, options);
          opened.push(path);
          return Effect.fail(
      ...
      const directory = `${home}/Library/Containers/com.apple.Safari/Data/Library/WebKit/WebsiteDataStore/11111111-1111-4111-8111-111111111111/Cookies`;
      yield* fileSystem.makeDirectory(directory, { recursive: true });
      const jar = `${directory}/Cookies.binarycookies`;
      yield* fileSystem.writeFileString(jar, "fixture");
      const sources = yield* importer.listSources;
      assert.equal(
        sources.find((source) => source.id === "safari")?.unavailable,
        "needsFullDiskAccess",
      );
      assert.deepEqual(opened, [jar]);
    ...
    Effect.gen(function* () {
      ...
      const error = yield* importer
        .importCookies({
          ...
          scope: "persist:pylon-code-preview-test",
          persistent: true,
        })
        .pipe(Effect.flip);
