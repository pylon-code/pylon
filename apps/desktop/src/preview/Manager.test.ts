...
describe("PreviewManager", () => {
  ...
  effectIt.effect("keeps preview shortcuts out of the host window", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        preview.reload.mockImplementation(() => reloaded.resolve());
  ...
  effectIt.effect("releases the tab lock when the host never answers the capture request", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const capturePage = vi.fn(async () => ({
          toJPEG: () => Buffer.from("recording-frame"),
        ...
        const host = makeTestHostWebContents();
        host.executeJavaScript.mockImplementation(() => new Promise(() => {}));
        fromId.mockReturnValue(makeTestPreviewWebContents(capturePage, 42, host));
        yield* manager.createTab("tab_wedged_host");
        yield* manager.registerWebview("tab_wedged_host", 42);
        yield* manager.setMainWindow({
          isDestroyed: () => false,
          once: vi.fn(),
          webContents: { setBackgroundThrottling: vi.fn() },
        ...
        const start = yield* Effect.exit(manager.startRecording("tab_wedged_host")).pipe(
          Effect.forkChild({ startImmediately: true }),
        );
        ...
        expect(Option.getOrThrow(Cause.findErrorOption(exit.cause))).toMatchObject({
          ...
          tabId: "tab_wedged_host",
        });
        // The failed start drops its arm, so a late renderer request is denied.
        const grants: Array<{ video?: unknown }> = [];
        host.displayMediaHandler()?.({ frame: host.mainFrame }, (value) => grants.push(value));
        expect(grants).toEqual([{}]);
        // The lifecycle lock is free again, so the tab can still close.
        yield* manager.closeTab("tab_wedged_host");
      }),
    ),
  );
  ...
  effectIt.effect("settles the pick when the annotation screenshot never arrives", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const startedPicks: Array<() => void> = [];
        const startPick = () => {
          const started = Promise.withResolvers<void>();
          startedPicks.push(started.resolve);
        ...
        fromId.mockReturnValue({
          ...
          send: (channel: string, ...args: unknown[]) => {
            ...
            if (channel === "preview:start-pick") startedPicks.shift()?.();
          },
          navigationHistory: { canGoBack: () => false, canGoForward: () => false },
          setWindowOpenHandler: vi.fn(),
          setIgnoreMenuShortcuts: vi.fn(),
