...
// The Electron calls in startRecording run under the tab lifecycle lock. A webContents
// with a wedged main thread — infinite loop, modal dialog, attached debugger — never
// settles them, which would hold the permit forever and turn every later closeTab
// into a silent no-op that leaks the webContents.
const RECORDING_SOURCE_WARM_TIMEOUT_MS = 2_000;
const RECORDING_CAPTURE_REQUEST_TIMEOUT_MS = 2_000;
    ...
    return yield* withTabLifecycleLock(
      ...
      Effect.gen(function* () {
        ...
        const captureRequested = yield* attemptPromise(
          ...
          Effect.timeout(Duration.millis(RECORDING_CAPTURE_REQUEST_TIMEOUT_MS)),
          // A renderer that never answers is reported as an unavailable capture below.
