...
const DesktopSettingsDocument = Schema.Struct({
  ...
  // `updateChannel` and `updateChannelConfiguredByUser` used to live here, back
  // when the update track was a preference. Stable and nightly are separate
  // applications now, so a build's track is fixed by the build. Documents still
  // carrying the old keys decode fine (excess keys are ignored) and shed them on
  // the next write.
});
