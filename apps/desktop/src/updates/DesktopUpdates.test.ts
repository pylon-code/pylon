...
describe("DesktopUpdates", () => {
  // Stable and nightly are separate applications with their own bundle ids and
  // runtime homes, so the channel a build follows is a property of the build.
  // It is never a preference, because no in-place update can turn one into the
  // other.
  it.effect("follows the stable feed on a stable build", () =>
    Effect.gen(function* () {
      const harness = makeHarness({ appVersion: "1.2.3" });
  ...
  it.effect("follows the nightly feed on a nightly build", () =>
    Effect.gen(function* () {
      const harness = makeHarness({ appVersion: "0.0.33-nightly.20260909.150" });
  ...
  it.effect("enables nightly full changelog release notes and broadcasts summaries", () => {
    const harness = makeHarness({ appVersion: "1.2.4-nightly.20260709.700" });
