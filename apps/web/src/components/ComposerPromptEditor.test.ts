import { COMPOSER_CONTEXT_CLIPBOARD_MIME } from "@t3tools/contracts";
...
describe("context reference paste", () => {
  it.each([
    ...
    ({ focus, prefix }) => {
      ...
      expect(shouldRoutePlainTextPaste(event.clipboardData, false)).toBe(false);
      expect(shouldRoutePlainTextPaste(event.clipboardData, true)).toBe(true);
      if (focus === "blurred") {
  ...
  it("imports an annotation's dependent screenshot when only its chip is pasted", () => {
    ...
    registerComposerInlineTokenPaste(editor, {
      ...
      importContextFragment: (fragment) => {
        imported.push(...fragment.records.map((record) => record.contextId));
        return new Map(fragment.records.map((record) => [record.contextId, record.contextId]));
      },
    });
...
describe("native composer syntax in structured paste", () => {
  it("retains mention paths and skill names through paste-to-focus", () => {
    const records = [
      { version: 1, kind: "mention", contextId: "mention", label: "File", path: "src/app.ts" },
      { version: 1, kind: "skill", contextId: "skill", label: "Skill", name: "review" },
    ];
    const event = new TestClipboardEvent(
      "Read [File](t3-context://v1/mention/mention) with [Skill](t3-context://v1/skill/skill)",
      {
    ...
    expect(importPastedComposerText(event.clipboardData, () => new Map())).toBe(
      "Read [app.ts](src/app.ts) with $review",
    );
  });
  it("claims a skill-only structured paste instead of falling through to its unresolved original link", () => {
    vi.stubGlobal("ClipboardEvent", TestClipboardEvent);
    const editor = createEditor();
    editor.update(
      () => {
    ...
    registerComposerInlineTokenPaste(editor, {
      createMentionNode: (path) => $createTextNode(path),
      createCitationNode: $createComposerCitationNode,
      createContextReferenceNode: (reference) => $createTextNode(reference.contextId),
      getExpandedAbsoluteOffsetForPoint: (_node, offset) => offset,
      importContextFragment: () => new Map(),
    });
    const event = new TestClipboardEvent("[Skill](t3-context://v1/skill/skill)", {
      [COMPOSER_CONTEXT_CLIPBOARD_MIME]: JSON.stringify({
        version: 1,
        source: { environmentId: "source" },
        records: [
...
describe("unavailable pasted context identity", () => {
  it.each(["missing", "malformed", "unknown", "not-imported"])(
    "keeps %s records from binding existing destination context",
    (mode) => {
      const contextId = "terminal_existing";
      const record =
        mode === "missing"
          ? []
          : mode === "malformed"
            ? [{ version: 1, kind: "terminal", contextId, label: "Missing terminal payload" }]
            : mode === "unknown"
              ? [{ version: 1, kind: "future", contextId, label: "Future", payload: {} }]
              : [
                  {
                    version: 1,
                    kind: "terminal",
      ...
      const kind = mode === "unknown" ? "future" : "terminal";
      const text = `[Unavailable](t3-context://v1/${kind}/${contextId})`;
      const event = new TestClipboardEvent(`${text} ${text}`, {
        [COMPOSER_CONTEXT_CLIPBOARD_MIME]: JSON.stringify({
          version: 1,
          source: { environmentId: "source" },
          records: record,
      ...
      const result = importPastedComposerText(event.clipboardData, () => new Map());
      expect(result).not.toContain(contextId);
      const refs = Array.from(
        result.matchAll(/t3-context:\/\/v1\/[^/]+\/([^)]*)/g),
        (match) => match[1],
      );
      expect(refs).toHaveLength(2);
      expect(refs[0]).toMatch(/^unavailable-/);
      expect(refs[0]).toBe(refs[1]);
      const destinationRecords = new Map([[contextId, "unrelated private context"]]);
      expect(refs.map((id) => destinationRecords.get(id!))).toEqual([undefined, undefined]);
    },
  );
  it("does not bind unaccompanied plain-text references to the destination", () => {
    const event = new TestClipboardEvent("[File](t3-context://v1/file/file_existing)");
    expect(importPastedComposerText(event.clipboardData, () => new Map())).not.toContain(
      "file_existing",
    );
  });
});
