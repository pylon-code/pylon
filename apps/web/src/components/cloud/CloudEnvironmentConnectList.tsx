...
/**
 * The user's Pylon Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through Pylon Connect.`,
      });
      return true;
    }
    ...
    const message =
      cause instanceof Error ? cause.message : "Could not connect the Pylon Connect environment.";
  ...
  return visibleEnvironments.map(({ environment, availability, error, status }) => {
    ...
    const statusText =
      unsupported && !savedEnvironment
        ? "Pylon Connect · Not added · Client not supported"
        : savedConnection
          ? savedConnection.statusText
          : availability === "online"
            ? "Pylon Connect · Not added · Relay online"
            : availability === "offline"
              ? "Pylon Connect · Not added · Relay offline"
              : availability === "checking"
                ? "Pylon Connect · Not added · Checking relay status…"
                : (Option.getOrNull(error)?.message ??
                  "Pylon Connect · Not added · Relay status unavailable");
    if (selection) {
      return (
        <label
