...
export interface PullRequestListOverride {
  ...
  readonly at: number;
  /** Only a completed action may be reconciled with a host answer. */
...
export function confirmPullRequestOverride(
  overrides: ReadonlyMap<string, PullRequestListOverride>,
  key: string,
  token: number,
): ReadonlyMap<string, PullRequestListOverride> {
  const override = overrides.get(key);
  if (override === undefined || override.token !== token || override.confirmed) return overrides;
  return new Map(overrides).set(key, { ...override, confirmed: true });
}
...
export function settlePullRequestOverrides<Entry extends PullRequestListEntry>(
  ...
  for (const [key, override] of overrides) {
    if (now - override.at > PULL_REQUEST_OVERRIDE_TRUST_MS) continue;
    ...
    if (!agrees) kept.set(key, override);
  }
