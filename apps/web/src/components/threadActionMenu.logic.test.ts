...
describe("buildThreadActionMenuItems", () => {
  ...
  it("offers project filtering only for the sidebar's scoped thread list", () => {
  ...
  it("offers a way back to all projects once the sidebar is scoped", () => {
    const items = buildThreadActionMenuItems({
      ...baseState,
      projectFilter: { label: "Beta Project", isActive: true },
    });
    const filterIndex = items.findIndex((item) => item.id === "filter-by-project");
