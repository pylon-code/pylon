...
export function canOpenMarkdownFileInEditor(
  canUseShellActions: boolean,
  availableEditors: ReadonlyArray<EditorId>,
): boolean {
  return canUseShellActions && availableEditors.length > 0;
}
...
export function resolveMarkdownFileRevealTarget(input: {
  filePath: string;
  cwd: string | undefined;
  workspaceBasenameMatch: string | null;
}): string {
  return input.workspaceBasenameMatch && input.cwd
    ? resolvePathLinkTarget(input.workspaceBasenameMatch, input.cwd)
    : input.filePath;
}
...
export const ChatMarkdownAssetImage = memo(function ChatMarkdownAssetImage(props: {
  ...
  /** Caps the image height while keeping its aspect ratio. */
      ...
      try {
        const clicked = await api.contextMenu.show(
          [
            ...
            ...(onOpen ? ([{ id: "open", label: getOpenInEditorMenuLabel() }] as const) : []),
            ...(onOpenInBrowser
...
function areMarkdownFileLinkPropsEqual(
  ...
  return (
    ...
    previous.getOpenInEditorMenuLabel === next.getOpenInEditorMenuLabel &&
...
function useChatMarkdownState({
  ...
  const canOpenInEditor = canOpenMarkdownFileInEditor(canUseShellActions, availableEditors);
  const getPreferredEditorMenuLabel = useCallback(
    () => openInEditorMenuLabel(resolvePreferredEditor(availableEditors)),
    [availableEditors],
  );
  ...
  const revealMarkdownFileInFileManager = useCallback(
    async (fileLinkMeta: MarkdownFileLinkMeta) => {
      ...
      const match = workspaceRelativePath
        ...
        : null;
      return revealFileInFileManager(
        resolveMarkdownFileRevealTarget({
          filePath: fileLinkMeta.filePath,
          cwd,
          workspaceBasenameMatch: match,
        }),
      );
    },
    ...
    (
      ...
      return (
        <MarkdownFileLink
          ...
          theme={resolvedTheme}
          threadRef={threadRef}
          {...(canOpenInEditor ? { onOpen: openInPreferredEditor } : {})}
          ...
          getOpenInEditorMenuLabel={getPreferredEditorMenuLabel}
          onReveal={
