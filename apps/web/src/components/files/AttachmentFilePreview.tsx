...
export function AttachmentFilePreview(props: {
  ...
  useEffect(() => {
    // oxlint-disable-next-line react/set-state-in-effect -- A replacement resource owns a new download action.
    setSaving(false);
    return () => {
      saveController.current?.abort();
      saveController.current = null;
    };
    // oxlint-disable-next-line react/exhaustive-effect-dependencies -- Replacing the resource cancels its pending Save even though cleanup only needs the controller.
  }, [props.file, props.asset?.environmentId, resource]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!file && Date.now() - authorizedAt.current > STALE_URL_MS) {
        const target = await refresh(signal);
        signal.throwIfAborted();
      ...
      const result = await readFilePreviewResponse(response, signal);
      signal.throwIfAborted();
      setContent(result);
    })().catch((cause: unknown) => {
      if (!controller.signal.aborted)
  ...
  const save = () => {
    ...
    void (async () => {
      try {
        let file = props.file;
        if (!file) {
          const target = await prepareDownload(signal);
          signal.throwIfAborted();
          if (!target) throw new Error("Reconnect to the environment and try again.");
