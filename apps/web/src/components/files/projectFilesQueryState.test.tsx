...
describe("project query refresh", () => {
  ...
  it("revalidates cached entries when a workspace mutation is observed after mounting", async () => {
    ...
    try {
      ...
      // Pylon adopts the first observed mutation without refreshing: the atom
      // has just issued its own mount read and refreshing would cancel it.
      render("mutation-1");
      expect(renderedPaths).toEqual(["src/old.ts"]);
      await flushEffects();
      expect(requests).toHaveLength(1);

      // A later mutation does refresh, once the coalesce window elapses.
      render("mutation-2");
      await flushEffects();
      vi.advanceTimersByTime(WORKSPACE_MUTATION_REFRESH_COALESCE_MS);
      await flushEffects();
