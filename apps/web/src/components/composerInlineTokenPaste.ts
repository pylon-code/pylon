...
/** Paste as Text wins over a copied rich fragment when the composer is blurred. */
export function shouldRoutePlainTextPaste(
  clipboardData: Pick<DataTransfer, "getData">,
  pasteAsText: boolean,
): boolean {
  return pasteAsText || (readPastedComposerContext(clipboardData)?.records.length ?? 0) === 0;
}
