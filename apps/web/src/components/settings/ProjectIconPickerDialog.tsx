...
export function ProjectIconPickerDialog({
  ...
  readonly monogramSupported: boolean;
  ...
  const [mode, setMode] = useState<ProjectIconOverride["kind"]>(
    current?.kind === "monogram" && !monogramSupported ? "lucide" : (current?.kind ?? "lucide"),
  );
  ...
  const [color, setColor] = useState<ProjectIconColor>(
    current && current.kind !== "emoji" ? current.color : DEFAULT_COLOR,
  );
  ...
  useEffect(() => {
    if (open && !previousOpenRef.current) {
      ...
      setColor(current && current.kind !== "emoji" ? current.color : DEFAULT_COLOR);
  ...
  }, [current, open, automatic.monogram, monogramSupported]);
  ...
  const validMonogram =
    isMonogramText(monogram) && Array.from(monogramSegmenter.segment(monogram)).length <= 2;
  const save = () => {
    if (mode === "monogram" && (!validMonogram || !monogramSupported)) return;
    onSelect(
      mode === "monogram"
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup className="w-full sm:w-[32rem]">
          ...
          <ToggleGroup
            ...
            onValueChange={(next) => {
              ...
              if (
                value === "lucide" ||
                value === "emoji" ||
                (value === "monogram" && monogramSupported)
              )
                setMode(value);
            }}
          >
            ...
            {monogramSupported ? <Toggle value="monogram">Monogram</Toggle> : null}
          </ToggleGroup>
        ...
        <DialogFooter>
          ...
          <Button
            onClick={save}
            disabled={mode === "monogram" && (!validMonogram || !monogramSupported)}
          >
            Save icon
          </Button>
        </DialogFooter>
