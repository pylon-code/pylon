...
function ProjectDetail({ group }: { group: SidebarProjectSnapshot }) {
  ...
  // New servers project an explicit null before an icon is chosen. Older servers omit the field.
  ...
  const setProjectIcon = useCallback(
    async (input: { faviconPath: string | null; projectIcon: ProjectIconOverride | null }) => {
      ...
      if (input.projectIcon !== null && !supportsProjectIcons) {
        toastManager.add({
          type: "warning",
          title: "Update every environment in this project group to save an icon.",
        });
        return;
      }
                ...
                <Button
                  ...
                  title={
                    supportsProjectIcons
                      ? undefined
                      : "Update every environment in this project group to save an icon."
                  }
                  disabled={isSavingFavicon || !supportsProjectIcons}
