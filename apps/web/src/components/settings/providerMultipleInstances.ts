...
export function getDriverMultipleInstancePresentation(input: {
  ...
  const supported =
    snapshots.length > 0 &&
    snapshots.every((provider) => provider.supportsMultipleInstances === true);
  const reason =
    snapshots.find((provider) => provider.supportsMultipleInstances !== true)
...
export function validatePrimeAgentAddHome(input: {
  ...
  const launchArgs = input.draftConfig.launchArgs;
  if (typeof launchArgs === "string" && launchArgs.trim().length > 0) {
    return `Custom Prime launch arguments require ACP compatibility. ${PRIME_AGENT_ACP_GUIDANCE}`;
  }
