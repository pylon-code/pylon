...
export function createDeviceStreamClient(
  ...
  // Browsers hide HTTP 401/403 handshake responses behind close code 1006.
  // Probe an allowlisted HTTP route before retrying the same media ticket.
  const retryAndroidAfterRefusedHandshake = async () => {
    const pending = new AbortController();
    authController = pending;
    const timeout = setTimeout(() => pending.abort(), 2_000);
    try {
      ...
      // A transport outage keeps the normal bounded reconnect cadence.
    } finally {
      clearTimeout(timeout);
      pending.abort();
      if (authController === pending) authController = null;
    }
    if (!stopped) scheduleRetry("input", connectAndroid);
  };
  ...
  const connectAndroid = () => {
    ...
    ws.onclose = (event) => {
      ...
      if (!stopped) {
        ...
        if (event.code === 1006) void retryAndroidAfterRefusedHandshake();
        else scheduleRetry("input", connectAndroid);
      }
    };
    ws.onerror = () => ws.close();
  };
