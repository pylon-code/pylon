...
describe("Android ticket expiry", () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });

  it("refreshes a rejected handshake ticket and reconnects with its replacement", async () => {
    vi.useFakeTimers();
    const sockets: FakeSocket[] = [];
    class FakeSocket {
      static OPEN = 1;
    ...
    const access = {
      ...
      wsBase: "ws://test/api/device-hub",
      credentials: false,
      query: { wsTicket: "expired" },
    };
    const unauthorized = vi.fn();
    const make = (wsTicket: string) =>
      createDeviceStreamClient(
        { platform: "android", deviceId: "pixel", access: { ...access, query: { wsTicket } } },
    ...
    const original = make("expired");
    original.start();
    sockets[0]!.onclose?.({ code: 1006, reason: "" });
    await vi.advanceTimersByTimeAsync(0);
    expect(fetcher.mock.calls[0]?.[0]).toContain("/vendor/serve-emu/health?wsTicket=expired");
    expect(unauthorized).toHaveBeenCalledOnce();
    await vi.advanceTimersByTimeAsync(3_000);
    expect(sockets).toHaveLength(1);
    const replacement = make("fresh");
    replacement.start();
    expect(sockets[1]?.url).toContain("wsTicket=fresh");
    replacement.stop();
  });
});
