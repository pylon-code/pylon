...
// This suite exercises disclosure and scroll coordination without mounting browser floating layers.
...
describe("MessagesTimeline", () => {
  ...
  it("hands the live slot to a shimmering Thinking row when the only tool failed", () => {
    const turnId = TurnId.make("turn-live");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
    ...
    // A failed call never owns the live slot, so nothing here carries the tool shine.
  ...
  it("renders initial thinking as the shared live activity row", () => {
    ...
    // A label-only row sweeps the masked overlay; the shine belongs to tool labels.
    expect(markup).toContain("live-activity-focus");
    expect(markup).not.toContain("live-tool-shine");
  });
