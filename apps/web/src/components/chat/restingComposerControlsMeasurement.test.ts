  ...
  const controls = {
    ...
    querySelectorAll: (selector: string) =>
      selector === "[data-resting-fixed-control]"
        ? input.fixedWidth
          ? [{ getBoundingClientRect: () => ({ width: input.fixedWidth }) }]
          : []
...
describe("measureRestingComposerControls", () => {
  ...
  it("reserves Pylon session controls before fitting the model and mode", () => {
    const measurement = measurePicker({ clientWidth: 0, flexGrow: "0", fixedWidth: 100 });
    expect(resolveRestingComposerControlsNaturalWidth(measurement)).toBe(300);
    expect(resolveRestingComposerControlsLayout({ ...measurement, hostWidth: 200 })).toEqual({
      hiddenCount: 1,
      visible: true,
    });
