...
describe("selected-message editing", () => {
  it("keeps a selected plan implementation instruction while excluding it from text-only recall", () => {
    const prompt = buildPlanImplementationPrompt("# Plan\n1. keep this instruction");
    expect(editableComposerPrompt(prompt)).toBe(prompt);
    expect(recallableComposerPrompt(prompt)).toBe("");
  });
});
