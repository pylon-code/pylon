...
describe("streaming row projection", () => {
  ...
  it("owns checkpoint lookups across streaming while preserving exact rollback proof", () => {
    const initial = fixture("Partial");
    ...
    const input = {
      ...initial.input,
      turnDiffSummaries: [baseline, checkpoint],
      supportsConversationRollback: true,
      expandedTurnIds: new Set([initial.historyTurnId]),
      expandedWorkGroupIds: new Set<string>(),
    };
    const firstUserRow = (projection: MessagesTimelineRowsProjection) =>
    ...
    const timeline = deriveTimelineEntriesWithState(messages, [], initial.work, initial.timeline);
    // Equal source snapshots arrive as fresh containers on every streamed token.
    const nextInput = {
      ...input,
      timelineEntries: timeline.entries,
    ...
    const revoked = deriveMessagesTimelineRowsWithState(
      {
        ...nextInput,
        turnDiffSummaries: [{ ...baseline, rollbackAvailability: undefined }, checkpoint],
      },
      next,
    );
    expect(firstUserRow(revoked)).toMatchObject({ revertTurnCount: undefined });
    const unsupported = deriveMessagesTimelineRowsWithState(
      { ...nextInput, supportsConversationRollback: false },
      next,
    );
  ...
  it("reuses long-thread rows through detail events, selectors, and attachment previews", () => {
    ...
    const history = Array.from({ length: 250 }, (_, index) => {
      ...
      const checkpoint: TurnDiffSummary = {
        ...
        checkpointRef: CheckpointRef.make(`refs/pylon/checkpoints/older-${index}`),
        status: "ready",
        files: [],
        get assistantMessageId() {
          checkpointLookupReads += 1;
    ...
    try {
      ...
      expect(
        first.rows.find(
          (row) => row.kind === "message" && row.message.id === MessageId.make("older-user-3"),
        ),
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps the first assistant message when the turn ends with no text", () => {
    const rows = deriveMessagesTimelineRows({
      timelineEntries: [
        {
          id: "assistant-first-entry",
    ...
    // Folding here would leave "Worked for 6s" above an empty response with the
    // answer nowhere on screen.
    expect(rows.map((row) => row.id)).toContain("assistant-first-entry");
  });
  ...
  it("keeps a missing-response work row visible outside settled-turn folding", () => {
    ...
    // The notice keeps its own row outside the fold. Grouping renders it as a
    // toggle, and a lone non-tool entry surfaces its own label as the summary,
    // so the message itself is still what the user reads.
    expect(rows.map((row) => row.id)).toEqual([
      "turn-fold:turn-1",
      "work-toggle:missing-response-entry",
    ]);
  ...
  it("keeps a promptless restart in one active visual response", () => {
    ...
    expect(rows.findIndex((row) => row.id === "working-indicator-row")).toBeLessThan(
      rows.findIndex((row) => row.kind === "work" && row.id === "old-work-entry"),
    );
