...
function ExpandedVideo({ item }: { readonly item: ExpandedImageItem }) {
  ...
  const connection = usePreparedConnection(asset?.environmentId ?? null);
  ...
  return (
    <MediaVideoPlayer
      ...
      label={item.name}
      sourceFailed={
        assetUrl._tag === "Failure" || (asset !== undefined && connection._tag === "None")
      }
...
export const ExpandedImageDialog = memo(function ExpandedImageDialog({
  ...
  const index = wrapExpandedImageIndex(preview.index + imageOffset, imageCount);
  ...
  const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    if (
      ...
      (event.target instanceof HTMLElement && event.target.tagName === "PRE")
    )
      return;
  ...
  return (
    <Dialog
      ...
      onOpenChange={(open, details) => {
        if (!open && details.reason === "escape-key" && isContextMenuOpen()) {
          details.cancel();
          return;
        }
        if (!open) onClose();
