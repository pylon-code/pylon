...
export function recallableComposerPrompt(messageText: string): string {
  return composerPromptFromMessage(messageText, false);
}

/** Edit from here restores backed context alongside the original prompt. */
export function editableComposerPrompt(messageText: string): string {
  return composerPromptFromMessage(messageText, true);
}

function composerPromptFromMessage(messageText: string, keepContextReferences: boolean): string {
  ...
  for (const reference of keepContextReferences
    ? []
    : collectComposerContextReferences(prompt).toReversed()) {
  ...
  if (
    ...
    (!keepContextReferences && trimmed.startsWith(PLAN_IMPLEMENTATION_PROMPT_PREFIX))
  ) {
    return "";
  }
  return trimmed;
