...
export function measureRestingComposerControls(
  ...
  // Pylon's quick-question, queue and harness controls remain inline. Their
  // width must be reserved before deciding whether the model and mode fit.
  const fixedControls = Array.from(
    controls.querySelectorAll<HTMLElement>("[data-resting-fixed-control]"),
  );
  const fixedWidth = fixedControls.reduce((total, control) => {
    const width = elementOuterWidth(control);
