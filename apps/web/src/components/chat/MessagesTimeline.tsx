...
import { useAssetUrlState, useAssetUrlRefresh, useAssetUrls } from "../../assets/assetUrls";
import {
  ...
  return (
    <>
      <div className="relative min-w-0 px-1 py-0.5">
        <MessageAuthorHeading>Pylon</MessageAuthorHeading>
        <AssistantCitationSource
          messageId={row.message.id}
  ...
  return (
    <div
      className={cn(
        ...
        alwaysVisible || reportedCostLabel
          ? "opacity-100"
    ...
    >
      ...
      {reportedCostLabel ? (
        <p className="text-muted-foreground text-xs tabular-nums">{reportedCostLabel}</p>
      ) : null}
    </div>
  );
}
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  return (
    ...
    >
      ...
      {expanded && canExpand && (expandedBody || viewedImage) && !workEntry.questionAnswer ? (
        <div
...
function UserVideoAttachment({ file }: { readonly file: ChatFileAttachment }) {
  ...
  const connection = usePreparedConnection(ctx.activeThreadEnvironmentId);
  ...
  return (
    <MediaVideoPlayer
      ...
      sourceFailed={
        ...
        resource !== null &&
        (assetUrl._tag === "Failure" || connection._tag === "None")
      }
      label={file.name}
      preload="visible"
      onOpen={() => {
