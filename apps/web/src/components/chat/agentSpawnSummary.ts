...
export function deriveAgentSpawnSummary({
  ...
  const active = agents.filter((agent) => isActiveSubagentStatus(agent.status)).length;
  ...
  const live =
    coordinatorStatus !== undefined ? !isTerminalSubagentStatus(coordinatorStatus) : active > 0;
  const subjects = [
