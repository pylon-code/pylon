...
describe("sidebar list motion", () => {
  ...
  it("gives a simple appended row its own travel when existing rows stay still", () => {
    const existing = new TestRow("existing", 82);
    const incoming = new TestRow("incoming", 82);
    const { motion, layout } = fixture([existing]);
    motion.update(true);
    layout([existing, incoming]);
    motion.update(true);

    expect(existing.animate).not.toHaveBeenCalled();
    expect(incoming.animate).toHaveBeenCalledWith(
      [
  ...
  it("does not ride a distant moving row across a stationary neighbour", () => {
    const moving = new TestRow("moving");
    const stationary = new TestRow("stationary");
    const incoming = new TestRow("incoming");
    const { motion, layout } = fixture([moving, stationary]);
    motion.update(true);
    layout([moving, stationary, incoming]);
    moving.offsetTop -= 40;
    motion.update(true);

    expectMove(moving, 40);
