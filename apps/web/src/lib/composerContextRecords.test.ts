...
describe("clipboard annotation screenshot ownership", () => {
  it.each([undefined, "image_missing"])(
    "never binds an absent screenshot record (%s) to an unrelated image with the annotation producer ID",
    (screenshotContextId) => {
      const record = previewAnnotationContextRecord(annotation, { screenshotContextId });
      const imported = previewAnnotationForContextImport(record, "fresh-clipboard-annotation");
      const message = buildMessageContext({
        terminalContexts: [],
        reviewComments: [],
        previewAnnotations: [imported],
        attachments: [
          {
            attachment: {
              type: "image",
              ...
              mimeType: "image/png",
              sizeBytes: 1,
              previewUrl: "blob:unrelated",
              file: new File(["x"], "unrelated.png"),
            },
            attachmentId: "unrelated-upload",
          },
        ],
      });
      const importedRecord = message?.records.find((entry) => entry.kind === "preview-annotation");
      expect(importedRecord).toMatchObject({ annotationId: "fresh-clipboard-annotation" });
      expect(importedRecord).not.toHaveProperty("screenshotContextId");
    },
  );
});
