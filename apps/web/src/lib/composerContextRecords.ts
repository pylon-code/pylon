...
/** Fresh clipboard annotations cannot inherit an unrelated draft image with the producer's ID. */
export function previewAnnotationForContextImport(
  record: PreviewAnnotationContextRecord,
  freshId?: string,
): PreviewAnnotationPayload {
  const annotation = previewAnnotationFromRecord(record);
  return freshId === undefined ? annotation : { ...annotation, id: freshId };
}
