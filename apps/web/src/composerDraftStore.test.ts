...
describe("composerDraftStore modelSelection", () => {
  ...
  it("durably blocks a cross-client binding without changing content or explicit routing", async () => {
    ...
    expect(hydratedDraft?.prompt).toBe(
      `keep this prompt ${formatInlineContextReference(fileContextReference(file))} `,
    );
  ...
  it("atomically transfers a complete blocked composer snapshot with its exact selection", async () => {
    ...
    const elementAnnotation = elementContextToPreviewAnnotation(
      {
        pageUrl: "https://example.com/dashboard",
        pageTitle: "Dashboard",
        tagName: "button",
      ...
      "move-conflict-element",
      "2026-09-02T10:00:00.000Z",
    );
    store.addPreviewAnnotation(threadRef, elementAnnotation);
    ...
    const sourcePrompt = store.getComposerDraft(threadRef)?.prompt;
    ...
    expect(destination?.previewAnnotations).toEqual([elementAnnotation, previewAnnotation]);
    ...
    expect(hydratedDestination?.previewAnnotations).toEqual([elementAnnotation, previewAnnotation]);
