...
describe("persistClientSettingsPatch", () => {
  it("waits for settings writes in request order", async () => {
    ...
    const first = persistClientSettingsPatch({ snapShotFlash: false }, persist);
    await firstStarted;
    const second = persistClientSettingsPatch({ snapShotPlaySound: false }, persist);
    ...
    expect(persist).toHaveBeenNthCalledWith(1, firstSettings);
    expect(persist).toHaveBeenNthCalledWith(2, secondSettings);
  });
});
