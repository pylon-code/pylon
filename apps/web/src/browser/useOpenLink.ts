    ...
    async (url, options = {}) => {
      ...
      if (target === "app" && targetThreadRef) {
        try {
          ...
          if (cause instanceof BrowserSettingsReadError) throw cause;
          console.error(cause);
        }
      }
      ...
      try {
        ...
        // Caller toasts may display this message; bridge errors can include private URLs.
        throw new Error("Unable to open link.", { cause });
      }
    },
