...
export interface ComposerContextAddOptions {
  /** False when the caller already placed the chip. */
  appendReference?: boolean;
  /** True when adding another reference to an existing record. */
  allowDuplicateReference?: boolean;
  /** False to append synchronously instead of using the mounted editor's caret. */
  insertAtCaret?: boolean;
}

/** Runtime-only hook: panel context lands at the mounted editor caret, otherwise it appends. */
...
/** Subscribe only to draft presence so sidebar rows do not rerender on each keystroke. */
