...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(yield* response.text).toContain("Hello from Astro on Prisma!");
  }),
  { timeout: 120_000 },
);
...
test(
  ...
  Effect.gen(function* () {
    ...
    expect(yield* response.json).toEqual({
      framework: "Astro",
      greeting: "Hello from Astro on Prisma!",
    });
  }),
  { timeout: 120_000 },
);
...
test(
  ...
  Effect.gen(function* () {
    ...
    const response = yield* Test.getWhenReady(url);
    expect(response.status).toBe(200);
    expect(yield* response.text).toContain("Static Astro on Prisma");
